            ...
            \DB::transaction(function() use ($request, $game)
            {
                try
                {
                    ...
                    if( $postData['slotEvent'] == 'doInit' ) 
                    {
                        ...
                        $fsInfo = '';
                        $slotSettings->SetGameData('BookofTutBonusWin', 0);
                        $slotSettings->SetGameData('BookofTutFreeGames', 0);
                        $slotSettings->SetGameData('BookofTutCurrentFreeGame', 0);
                        $slotSettings->SetGameData('BookofTutTotalWin', 0);
                        $slotSettings->SetGameData($slotSettings->slotId . 'FreeBalance', $slotSettings->GetBalance());
                        $slotSettings->SetGameData('BookofTutBonusState', 0);
                        $slotSettings->SetGameData('BookofTutBonusMpl', 0);
                        if( $lastEvent != 'NULL' ) 
                        {
                    ...
                    else if( $postData['slotEvent'] == 'gamble5GetUserCards' ) 
                    {
                        ...
                        $dealerCard = $slotSettings->GetGameData('BookofTutDealerCard');
                        $totalWin = $slotSettings->GetGameData('BookofTutTotalWin');
                        $gambleWin = 0;
                        ...
                        $slotSettings->SetGameData('BookofTutTotalWin', $totalWin);
                    ...
                    else if( $postData['slotEvent'] == 'gamble5GetDealerCard' ) 
                    {
                        ...
                        $slotSettings->SetGameData('BookofTutDealerCard', $tmpDc);
                    ...
                    {
                        $bstate = $slotSettings->GetGameData('BookofTutBonusState');
                        $Balance = $slotSettings->GetBalance();
                        if( $bstate == 0 ) 
                        {
                            if( $postData['ind'] == 1 ) 
                            ...
                            $curWin = $slotSettings->GetGameData('BookofTutBonusWin');
                            $curBet = $slotSettings->GetGameData('BookofTutAllBet');
                        ...
                        $slotSettings->SetGameData('BookofTutBonusState', $bstate);
                    }
                    else if( $postData['slotEvent'] == 'doMysteryScatter' ) 
                    {
                        $Balance = $slotSettings->GetBalance();
                        $rSym = rand(3, 11);
                        $slotSettings->SetGameData('BookofTutFreeGames', $slotSettings->slotFreeCount);
                        $slotSettings->SetGameData('BookofTutCurrentFreeGame', 0);
                        $slotSettings->SetGameData('BookofTutSym', $rSym);
                    ...
                    {
                        if( $slotSettings->GetGameData('BookofTutCurrentFreeGame') < $slotSettings->GetGameData('BookofTutFreeGames') && $slotSettings->GetGameData('BookofTutFreeGames') > 0 ) 
                        {
                        ...
                        if( $postData['slotEvent'] != 'freespin' ) 
                        {
                            ...
                            $slotSettings->SetGameData('BookofTutBonusState', 0);
                            $slotSettings->SetGameData('BookofTutBonusMpl', 0);
                            $slotSettings->SetGameData('BookofTutSym', 0);
                            $slotSettings->SetGameData('BookofTutAllBet', $postData['slotBet'] * $postData['slotLines']);
                        }
                        else
                        {
                            $slotSettings->SetGameData('BookofTutCurrentFreeGame', $slotSettings->GetGameData('BookofTutCurrentFreeGame') + 1);
                            $bonusMpl = $slotSettings->GetGameData('BookofTutBonusMpl');
                        }
                        $Balance = $slotSettings->GetBalance();
                        if( $postData['slotEvent'] != 'freespin' ) 
                        {
                        ...
                        for( $i = 0; $i <= 2000; $i++ ) 
                        {
                            ...
                            for( $r = 1; $r <= 5; $r++ ) 
                            {
                                for( $p = 0; $p <= 2; $p++ ) 
                                {
                                    ...
                                    if( $reels['reel' . $r][$p] == $slotSettings->GetGameData('BookofTutSym') ) 
                                    {
                                        $scattersCount2++;
                            ...
                            for( $r = 1; $r <= 5; $r++ ) 
                            {
                                for( $p = 0; $p <= 2; $p++ ) 
                                {
                                    if( $reels2['reel' . $r][$p] == $slotSettings->GetGameData('BookofTutSym') ) 
                                    {
                                        $reels2['reel' . $r][0] = $slotSettings->GetGameData('BookofTutSym');
                                        $reels2['reel' . $r][1] = $slotSettings->GetGameData('BookofTutSym');
                                        $reels2['reel' . $r][2] = $slotSettings->GetGameData('BookofTutSym');
                                        break;
                                    }
                                }
                            }
                            ...
                            if( isset($slotSettings->Paytable[$slotSettings->GetGameData('BookofTutSym')]) && $slotSettings->Paytable[$slotSettings->GetGameData('BookofTutSym')][$scattersCount2] > 0 ) 
                            {
                                $scattersWin2 = $slotSettings->Paytable['' . $slotSettings->GetGameData('BookofTutSym')][$scattersCount2] * $postData['slotBet'] * $postData['slotLines'] * $bonusMpl;
                            }
                            else
                            {
                                $scattersWin2 = 0;
                            ...
                            if( $scattersWin2 > 0 ) 
                            {
                                $winString .= ('&psym=' . $slotSettings->GetGameData('BookofTutSym') . '~' . $scattersWin . '~' . implode(',', $scattersStr));
                        ...
                        if( $scattersCount >= 3 ) 
                        {
                            if( $slotSettings->GetGameData('BookofTutFreeGames') == 0 ) 
                            {
                                $slotSettings->SetGameData('BookofTutBonusMpl', 1);
                                $spinType = 'b&status=0,0,0&rw=0.00&wins=0,0,0&bw=1&wins_mask=h,h,h&wp=0&end=0&bgt=28';
                            }
                            else
                            {
                                $slotSettings->SetGameData('BookofTutFreeGames', $slotSettings->GetGameData('BookofTutFreeGames') + $slotSettings->slotFreeCount);
                            }
                        }
                        if( $postData['slotEvent'] == 'freespin' ) 
                        {
                            $slotSettings->SetGameData('BookofTutBonusWin', $slotSettings->GetGameData('BookofTutBonusWin') + $totalWin);
                            $slotSettings->SetGameData('BookofTutTotalWin', $slotSettings->GetGameData('BookofTutTotalWin') + $totalWin);
                            $spinType = 's';
                            $Balance = $slotSettings->GetGameData('BookofTutFreeBalance');
                            if( $slotSettings->GetGameData('BookofTutFreeGames') <= $slotSettings->GetGameData('BookofTutCurrentFreeGame') && $slotSettings->GetGameData('BookofTutFreeGames') > 0 ) 
                            {
                                $spinType = 'c';
                                $winString .= ('&ms=' . $slotSettings->GetGameData('BookofTutSym') . '&fs_total=' . $slotSettings->GetGameData('BookofTutFreeGames') . '&fsmul_total=' . $slotSettings->GetGameData('BookofTutBonusMpl') . '&fswin_total=' . $slotSettings->GetGameData('BookofTutBonusWin') . '&fsres_total=' . $slotSettings->GetGameData('BookofTutBonusWin') . '');
                            }
                            else
                            {
                                $winString .= ('&ms=' . $slotSettings->GetGameData('BookofTutSym') . '&fsmul=' . $slotSettings->GetGameData('BookofTutBonusMpl') . '&fsmax=' . $slotSettings->GetGameData('BookofTutFreeGames') . '&fswin=' . $slotSettings->GetGameData('BookofTutTotalWin') . '&fs=' . $slotSettings->GetGameData('BookofTutCurrentFreeGame') . '&fsres=' . $slotSettings->GetGameData('BookofTutBonusWin'));
                            }
                        ...
                        else
                        {
                            ...
                            $slotSettings->SetGameData('BookofTutTotalWin', $totalWin);
                            $slotSettings->SetGameData('BookofTutBonusWin', $totalWin);
                        }
                        ...
                        $response = 'tw=' . $slotSettings->GetGameData('BookofTutBonusWin') . '&bstate=' . $slotSettings->GetGameData('BookofTutBonusState') . '&balance=' . $Balance . '&index=' . $postData['index'] . '&balance_cash=' . $Balance . '&balance_bonus=0.00' . $winString . '&na=' . $spinType . '&stime=' . floor(microtime(true) * 1000) . '&sa=' . $reels['reel1'][3] . ',' . $reels['reel2'][3] . ',' . $reels['reel3'][3] . ',' . $reels['reel4'][3] . ',' . $reels['reel5'][3] . '&sb=' . $reels['reel1'][-1] . ',' . $reels['reel2'][-1] . ',' . $reels['reel3'][-1] . ',' . $reels['reel4'][-1] . ',' . $reels['reel5'][-1] . '&sh=3&c=0.01&sver=5&n_reel_set=0&counter=' . ((int)$postData['counter'] + 1) . '&l=10&s=' . $s . '&w=' . $totalWinRaw . '';
                        if( $slotSettings->GetGameData('BookofTutFreeGames') <= $slotSettings->GetGameData('BookofTutCurrentFreeGame') && $slotSettings->GetGameData('BookofTutFreeGames') > 0 ) 
                        {
                            $slotSettings->SetGameData('BookofTutTotalWin', 0);
                            $slotSettings->SetGameData('BookofTutBonusWin', 0);
                        }
                        $response_log = '{"responseEvent":"spin","responseType":"' . $postData['slotEvent'] . '","serverResponse":{"Sym":' . $slotSettings->GetGameData('BookofTutSym') . ',"BonusMpl":' . $slotSettings->GetGameData('BookofTutBonusMpl') . ',"lines":' . $postData['slotLines'] . ',"bet":' . $postData['slotBet'] . ',"totalFreeGames":' . $slotSettings->GetGameData('BookofTutFreeGames') . ',"currentFreeGames":' . $slotSettings->GetGameData('BookofTutCurrentFreeGame') . ',"Balance":' . $Balance . ',"afterBalance":' . $slotSettings->GetBalance() . ',"totalWin":' . $totalWin . ',"bonusWin":' . $slotSettings->GetGameData('BookofTutBonusWin') . ',"winLines":[],"Jackpots":' . $jsJack . ',"reelsSymbols":' . $jsSpin . '}}';
                        ...
                        if( $scattersCount >= 3 ) 
                        {
                            ...
                            {
                                $slotSettings->SetGameData('BookofTutFreeBalance', $Balance);
                                $slotSettings->SetGameData('BookofTutTotalWin', 0);
                                $slotSettings->SetGameData('BookofTutBonusState', 0);
                                $slotSettings->SetGameData('BookofTutBonusWin', $totalWin);
                            }
