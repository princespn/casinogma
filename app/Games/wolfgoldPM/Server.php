<?php 
namespace VanguardLTE\Games\wolfgoldPM
{
    set_time_limit(5);
    class Server
    {
            ...
            \DB::transaction(function() use ($request, $game)
            {
                try
                {
                    ...
                    if( $postData['slotEvent'] == 'doInit' ) 
                    {
                        ...
                        $fsInfo = '';
                        $slotSettings->SetGameData('wolfgoldPMBonusWin', 0);
                        $slotSettings->SetGameData('wolfgoldPMFreeGames', 0);
                        $slotSettings->SetGameData('wolfgoldPMCurrentFreeGame', 0);
                        $slotSettings->SetGameData('wolfgoldPMTotalWin', 0);
                        $slotSettings->SetGameData($slotSettings->slotId . 'FreeBalance', $slotSettings->GetBalance());
                        $slotSettings->SetGameData('wolfgoldPMBonusState', 0);
                        $slotSettings->SetGameData('wolfgoldPMBonusMpl', 0);
                        if( $lastEvent != 'NULL' ) 
                        {
                    ...
                    else if( $postData['slotEvent'] == 'gamble5GetUserCards' ) 
                    {
                        ...
                        $dealerCard = $slotSettings->GetGameData('wolfgoldPMDealerCard');
                        $totalWin = $slotSettings->GetGameData('wolfgoldPMTotalWin');
                        $gambleWin = 0;
                        ...
                        $slotSettings->SetGameData('wolfgoldPMTotalWin', $totalWin);
                    ...
                    else if( $postData['slotEvent'] == 'gamble5GetDealerCard' ) 
                    {
                        ...
                        $slotSettings->SetGameData('wolfgoldPMDealerCard', $tmpDc);
                    ...
                    {
                        $bstate = $slotSettings->GetGameData('wolfgoldPMBonusState');
                        $Balance = $slotSettings->GetBalance();
                        if( $bstate == 0 ) 
                        {
                            $andFree = [
                            ...
                            $andFree[$postData['ind']] = $slotSettings->GetGameData('wolfgoldPMFreeGames');
                            $stat[$postData['ind']] = 1;
                        ...
                        else if( $bstate == 1 ) 
                        {
                            ...
                            $andFree[$postData['ind']] = $slotSettings->GetGameData('wolfgoldPMBonusMpl');
                            $stat[$postData['ind']] = 2;
                            $response = 'fsmul=' . $slotSettings->GetGameData('wolfgoldPMBonusMpl') . '&bgid=0&balance=' . $Balance . '&wins=' . implode(',', $andFree) . '&fsmax=' . $slotSettings->GetGameData('wolfgoldPMFreeGames') . '&level=2&index=149&balance_cash=' . $Balance . '&balance_bonus=0.00&na=s&fswin=0.00&status=' . implode(',', $stat) . '&stime=' . floor(microtime(true) * 1000) . '&fs=1&bgt=28&wins_mask=m,m,m,m,m,m&end=1&fsres=0.00&sver=5&n_reel_set=1&counter=298';
                            $bstate++;
                        }
                        $slotSettings->SetGameData('wolfgoldPMBonusState', $bstate);
                    }
                    ...
                    {
                        if( $slotSettings->GetGameData('wolfgoldPMCurrentFreeGame') < $slotSettings->GetGameData('wolfgoldPMFreeGames') && $slotSettings->GetGameData('wolfgoldPMFreeGames') > 0 ) 
                        {
                        ...
                        {
                            $slotSettings->SetGameData('wolfgoldPMCurrentFreeGame', $slotSettings->GetGameData('wolfgoldPMCurrentFreeGame') + 1);
                            $bonusMpl = $slotSettings->GetGameData('wolfgoldPMBonusMpl');
                        }
                        $Balance = $slotSettings->GetBalance();
                        if( $postData['slotEvent'] != 'freespin' ) 
                        {
                        ...
                        if( $scattersCount >= 3 ) 
                        {
                            if( $slotSettings->GetGameData('wolfgoldPMFreeGames') == 0 ) 
                            {
                                $slotSettings->SetGameData('wolfgoldPMBonusMpl', rand(1, 18));
                                $slotSettings->SetGameData('wolfgoldPMFreeGames', rand(10, 38));
                                $spinType = 'b&bw=1&bgid=0&bgt=28';
                            }
                            else
                            {
                                $slotSettings->SetGameData('wolfgoldPMFreeGames', $slotSettings->GetGameData('wolfgoldPMFreeGames') + $slotSettings->slotFreeCount);
                            }
                        }
                        if( $postData['slotEvent'] == 'freespin' ) 
                        {
                            $slotSettings->SetGameData('wolfgoldPMBonusWin', $slotSettings->GetGameData('wolfgoldPMBonusWin') + $totalWin);
                            $slotSettings->SetGameData('wolfgoldPMTotalWin', $slotSettings->GetGameData('wolfgoldPMTotalWin') + $totalWin);
                            $spinType = 's';
                            $Balance = $slotSettings->GetGameData('wolfgoldPMFreeBalance');
                            if( $slotSettings->GetGameData('wolfgoldPMFreeGames') <= $slotSettings->GetGameData('wolfgoldPMCurrentFreeGame') && $slotSettings->GetGameData('wolfgoldPMFreeGames') > 0 ) 
                            {
                                $spinType = 'c';
                                $winString .= ('&fs_total=' . $slotSettings->GetGameData('wolfgoldPMFreeGames') . '&fsmul_total=' . $slotSettings->GetGameData('wolfgoldPMBonusMpl') . '&fswin_total=' . $slotSettings->GetGameData('wolfgoldPMBonusWin') . '&fsres_total=' . $slotSettings->GetGameData('wolfgoldPMBonusWin') . '');
                            }
                            else
                            {
                                $winString .= ('&fsmul=' . $slotSettings->GetGameData('wolfgoldPMBonusMpl') . '&fsmax=' . $slotSettings->GetGameData('wolfgoldPMFreeGames') . '&fswin=' . $slotSettings->GetGameData('wolfgoldPMTotalWin') . '&fs=' . $slotSettings->GetGameData('wolfgoldPMCurrentFreeGame') . '&fsres=' . $slotSettings->GetGameData('wolfgoldPMBonusWin'));
                            }
                        ...
                        else
                        {
                            ...
                            $slotSettings->SetGameData('wolfgoldPMTotalWin', $totalWin);
                            $slotSettings->SetGameData('wolfgoldPMBonusWin', $totalWin);
                        }
                        ...
                        $response = 'tw=' . $slotSettings->GetGameData('wolfgoldPMBonusWin') . '&bstate=' . $slotSettings->GetGameData('wolfgoldPMBonusState') . '&balance=' . $Balance . '&index=' . $postData['index'] . '&balance_cash=' . $Balance . '&balance_bonus=0.00' . $winString . '&na=' . $spinType . '&stime=' . floor(microtime(true) * 1000) . '&sa=' . $reels['reel1'][3] . ',' . $reels['reel2'][3] . ',' . $reels['reel3'][3] . ',' . $reels['reel4'][3] . ',' . $reels['reel5'][3] . '&sb=' . $reels['reel1'][-1] . ',' . $reels['reel2'][-1] . ',' . $reels['reel3'][-1] . ',' . $reels['reel4'][-1] . ',' . $reels['reel5'][-1] . '&sh=3&c=0.01&sver=5&n_reel_set=0&counter=' . ((int)$postData['counter'] + 1) . '&l=25&s=' . $s . '&w=' . $totalWinRaw . '';
                        if( $slotSettings->GetGameData('wolfgoldPMFreeGames') <= $slotSettings->GetGameData('wolfgoldPMCurrentFreeGame') && $slotSettings->GetGameData('wolfgoldPMFreeGames') > 0 ) 
                        {
                            $slotSettings->SetGameData('wolfgoldPMTotalWin', 0);
                            $slotSettings->SetGameData('wolfgoldPMBonusWin', 0);
                        }
                        $response_log = '{"responseEvent":"spin","responseType":"' . $postData['slotEvent'] . '","serverResponse":{"BonusMpl":' . $slotSettings->GetGameData('wolfgoldPMBonusMpl') . ',"lines":' . $postData['slotLines'] . ',"bet":' . $postData['slotBet'] . ',"totalFreeGames":' . $slotSettings->GetGameData('wolfgoldPMFreeGames') . ',"currentFreeGames":' . $slotSettings->GetGameData('wolfgoldPMCurrentFreeGame') . ',"Balance":' . $Balance . ',"afterBalance":' . $slotSettings->GetBalance() . ',"totalWin":' . $totalWin . ',"bonusWin":' . $slotSettings->GetGameData('wolfgoldPMBonusWin') . ',"winLines":[],"Jackpots":' . $jsJack . ',"reelsSymbols":' . $jsSpin . '}}';
                        ...
                        if( $scattersCount >= 3 ) 
                        {
                            ...
                            {
                                $slotSettings->SetGameData('wolfgoldPMFreeBalance', $Balance);
                                $slotSettings->SetGameData('wolfgoldPMTotalWin', 0);
                                $slotSettings->SetGameData('wolfgoldPMBonusState', 0);
                                $slotSettings->SetGameData('wolfgoldPMBonusWin', $totalWin);
                            }
