...
namespace VanguardLTE\Games\chillePMM
{
    class SlotSettings
    {
        ...
        public function GetSpinSettings($garantType = 'doSpin', $bet, $lines)
        {
            ...
            $spinWin = 0;
            $game = $this->game;
            $curSpinStep = $game->{'garant_win' . $pref . $curField};
            $curBonusStep = $game->{'garant_bonus' . $pref . $curField};
            $limitBonus = $game->{'winbonus' . $pref . $curField};
            $limitWinline = $game->{'winline' . $pref . $curField};
            $curSpinStep++;
            $curBonusStep++;
            $return = [
                'none', 
            ...
            if( $limitBonus <= $curBonusStep ) 
            {
                $bonusWin = 1;
                $curBonusStep = 0;
                $game->{'winbonus' . $pref . $curField} = $this->getNewSpin($game, 0, 1, $lines, $garantType);
            }
            else if( $limitWinline <= $curSpinStep ) 
            {
                $spinWin = 1;
                $curSpinStep = 0;
                $game->{'winline' . $pref . $curField} = $this->getNewSpin($game, 1, 0, $lines, $garantType);
            }
            $game->{'garant_win' . $pref . $curField} = $curSpinStep;
            $game->{'garant_bonus' . $pref . $curField} = $curBonusStep;
            $game->save();
