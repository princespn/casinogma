<?php 
namespace VanguardLTE\Games\chillePMM
{
    set_time_limit(10);
            ...
            \DB::transaction(function() use ($request, $game)
            {
                try
                {
                    ...
                    if( $postData['slotEvent'] == 'doInit' ) 
                    {
                        ...
                        $fsInfo = '';
                        $slotSettings->SetGameData('chillePMMBonusWin', 0);
                        $slotSettings->SetGameData('chillePMMFreeGames', 0);
                        $slotSettings->SetGameData('chillePMMCurrentFreeGame', 0);
                        $slotSettings->SetGameData('chillePMMTotalWin', 0);
                        $slotSettings->SetGameData($slotSettings->slotId . 'FreeBalance', $slotSettings->GetBalance());
                        $slotSettings->SetGameData('chillePMMBonusState', 0);
                        $slotSettings->SetGameData('chillePMMBonusMpl', 0);
                        if( $lastEvent != 'NULL' ) 
                        {
                    ...
                    else if( $postData['slotEvent'] == 'gamble5GetUserCards' ) 
                    {
                        ...
                        $dealerCard = $slotSettings->GetGameData('chillePMMDealerCard');
                        $totalWin = $slotSettings->GetGameData('chillePMMTotalWin');
                        $gambleWin = 0;
                        ...
                        $slotSettings->SetGameData('chillePMMTotalWin', $totalWin);
                    ...
                    else if( $postData['slotEvent'] == 'gamble5GetDealerCard' ) 
                    {
                        ...
                        $slotSettings->SetGameData('chillePMMDealerCard', $tmpDc);
                    ...
                    {
                        $bstate = $slotSettings->GetGameData('chillePMMBonusState');
                        $Balance = $slotSettings->GetBalance();
                        if( $bstate == 0 ) 
                        {
                            $andFree = [
                            ...
                            $andFree[$postData['ind']] = $slotSettings->GetGameData('chillePMMFreeGames');
                            $stat[$postData['ind']] = 1;
                        ...
                        else if( $bstate == 1 ) 
                        {
                            ...
                            $andFree[$postData['ind']] = $slotSettings->GetGameData('chillePMMBonusMpl');
                            $stat[$postData['ind']] = 2;
                            $response = 'fsmul=' . $slotSettings->GetGameData('chillePMMBonusMpl') . '&bgid=0&balance=' . $Balance . '&wins=' . implode(',', $andFree) . '&fsmax=' . $slotSettings->GetGameData('chillePMMFreeGames') . '&level=2&index=149&balance_cash=' . $Balance . '&balance_bonus=0.00&na=s&fswin=0.00&status=' . implode(',', $stat) . '&stime=' . floor(microtime(true) * 1000) . '&fs=1&bgt=28&wins_mask=m,m,m,m,m,m&end=1&fsres=0.00&sver=5&n_reel_set=1&counter=298';
                            $bstate++;
                        }
                        $slotSettings->SetGameData('chillePMMBonusState', $bstate);
                    }
                    ...
                    {
                        if( $slotSettings->GetGameData('chillePMMCurrentFreeGame') < $slotSettings->GetGameData('chillePMMFreeGames') && $slotSettings->GetGameData('chillePMMFreeGames') > 0 ) 
                        {
                        ...
                        {
                            $slotSettings->SetGameData('chillePMMCurrentFreeGame', $slotSettings->GetGameData('chillePMMCurrentFreeGame') + 1);
                            $bonusMpl = $slotSettings->GetGameData('chillePMMBonusMpl');
                        }
                        $Balance = $slotSettings->GetBalance();
                        if( $postData['slotEvent'] != 'freespin' ) 
                        {
                        ...
                        for( $i = 0; $i <= 2000; $i++ ) 
                        {
                            ...
                            if( $slotSettings->winLimitsArr[$slotSettings->GetGameDataStatic($slotSettings->slotId . 'timeWinLimitNum')][2] < ($totalWin * $slotSettings->CurrentDenom) ) 
                            {
                            }
                            else
                            {
                                if( $slotSettings->winLimitsArr[$slotSettings->GetGameDataStatic($slotSettings->slotId . 'timeWinLimitNum')][1] < ($slotSettings->GetGameDataStatic($slotSettings->slotId . 'timeWin') + ($totalWin * $slotSettings->CurrentDenom)) ) 
                                {
                                    $winType = 'none';
                                }
                        ...
                        if( $scattersCount >= 3 ) 
                        {
                            if( $slotSettings->GetGameData('chillePMMFreeGames') == 0 ) 
                            {
                                $slotSettings->SetGameData('chillePMMBonusMpl', rand(1, 18));
                                $slotSettings->SetGameData('chillePMMFreeGames', rand(10, 38));
                                $spinType = 'b&bw=1&bgid=0&bgt=28';
                            }
                            else
                            {
                                $slotSettings->SetGameData('chillePMMFreeGames', $slotSettings->GetGameData('chillePMMFreeGames') + $slotSettings->slotFreeCount);
                            }
                        }
                        if( $postData['slotEvent'] == 'freespin' ) 
                        {
                            $slotSettings->SetGameData('chillePMMBonusWin', $slotSettings->GetGameData('chillePMMBonusWin') + $totalWin);
                            $slotSettings->SetGameData('chillePMMTotalWin', $slotSettings->GetGameData('chillePMMTotalWin') + $totalWin);
                            $spinType = 's';
                            $Balance = $slotSettings->GetGameData('chillePMMFreeBalance');
                            if( $slotSettings->GetGameData('chillePMMFreeGames') <= $slotSettings->GetGameData('chillePMMCurrentFreeGame') && $slotSettings->GetGameData('chillePMMFreeGames') > 0 ) 
                            {
                                $spinType = 'c';
                                $winString .= ('&fs_total=' . $slotSettings->GetGameData('chillePMMFreeGames') . '&fsmul_total=' . $slotSettings->GetGameData('chillePMMBonusMpl') . '&fswin_total=' . $slotSettings->GetGameData('chillePMMBonusWin') . '&fsres_total=' . $slotSettings->GetGameData('chillePMMBonusWin') . '');
                            }
                            else
                            {
                                $winString .= ('&fsmul=' . $slotSettings->GetGameData('chillePMMBonusMpl') . '&fsmax=' . $slotSettings->GetGameData('chillePMMFreeGames') . '&fswin=' . $slotSettings->GetGameData('chillePMMTotalWin') . '&fs=' . $slotSettings->GetGameData('chillePMMCurrentFreeGame') . '&fsres=' . $slotSettings->GetGameData('chillePMMBonusWin'));
                            }
                        ...
                        else
                        {
                            ...
                            $slotSettings->SetGameData('chillePMMTotalWin', $totalWin);
                            $slotSettings->SetGameData('chillePMMBonusWin', $totalWin);
                        }
                        ...
                        $response = 'tw=' . $slotSettings->GetGameData('chillePMMBonusWin') . '&bstate=' . $slotSettings->GetGameData('chillePMMBonusState') . '&balance=' . $Balance . '&index=' . $postData['index'] . '&balance_cash=' . $Balance . '&balance_bonus=0.00' . $winString . '&na=' . $spinType . '&stime=' . floor(microtime(true) * 1000) . '&sa=' . $reels['reel1'][3] . ',' . $reels['reel2'][3] . ',' . $reels['reel3'][3] . ',' . $reels['reel4'][3] . ',' . $reels['reel5'][3] . '&sb=' . $reels['reel1'][-1] . ',' . $reels['reel2'][-1] . ',' . $reels['reel3'][-1] . ',' . $reels['reel4'][-1] . ',' . $reels['reel5'][-1] . '&sh=3&c=0.01&sver=5&n_reel_set=0&counter=' . ((int)$postData['counter'] + 1) . '&l=25&s=' . $s . '&w=' . $totalWinRaw . '';
                        if( $slotSettings->GetGameData('chillePMMFreeGames') <= $slotSettings->GetGameData('chillePMMCurrentFreeGame') && $slotSettings->GetGameData('chillePMMFreeGames') > 0 ) 
                        {
                            $slotSettings->SetGameData('chillePMMTotalWin', 0);
                            $slotSettings->SetGameData('chillePMMBonusWin', 0);
                        }
                        $response_log = '{"responseEvent":"spin","responseType":"' . $postData['slotEvent'] . '","serverResponse":{"BonusMpl":' . $slotSettings->GetGameData('chillePMMBonusMpl') . ',"lines":' . $postData['slotLines'] . ',"bet":' . $postData['slotBet'] . ',"totalFreeGames":' . $slotSettings->GetGameData('chillePMMFreeGames') . ',"currentFreeGames":' . $slotSettings->GetGameData('chillePMMCurrentFreeGame') . ',"Balance":' . $Balance . ',"afterBalance":' . $slotSettings->GetBalance() . ',"totalWin":' . $totalWin . ',"bonusWin":' . $slotSettings->GetGameData('chillePMMBonusWin') . ',"winLines":[],"Jackpots":' . $jsJack . ',"reelsSymbols":' . $jsSpin . '}}';
                        ...
                        if( $scattersCount >= 3 ) 
                        {
                            ...
                            {
                                $slotSettings->SetGameData('chillePMMFreeBalance', $Balance);
                                $slotSettings->SetGameData('chillePMMTotalWin', 0);
                                $slotSettings->SetGameData('chillePMMBonusState', 0);
                                $slotSettings->SetGameData('chillePMMBonusWin', $totalWin);
                            }
