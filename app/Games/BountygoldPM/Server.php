            ...
            \DB::transaction(function() use ($request, $game)
            {
                try
                {
                    ...
                    if( $postData['slotEvent'] == 'doInit' ) 
                    {
                        ...
                        $fsInfo = '';
                        $slotSettings->SetGameData('BountygoldPMBonusWin', 0);
                        $slotSettings->SetGameData('BountygoldPMFreeGames', 0);
                        $slotSettings->SetGameData('BountygoldPMCurrentFreeGame', 0);
                        $slotSettings->SetGameData('BountygoldPMTotalWin', 0);
                        $slotSettings->SetGameData($slotSettings->slotId . 'FreeBalance', $slotSettings->GetBalance());
                        $slotSettings->SetGameData('BountygoldPMBonusState', 0);
                        $slotSettings->SetGameData('BountygoldPMBonusMpl', 0);
                        if( $lastEvent != 'NULL' ) 
                        {
                    ...
                    else if( $postData['slotEvent'] == 'gamble5GetUserCards' ) 
                    {
                        ...
                        $dealerCard = $slotSettings->GetGameData('BountygoldPMDealerCard');
                        $totalWin = $slotSettings->GetGameData('BountygoldPMTotalWin');
                        $gambleWin = 0;
                        ...
                        $slotSettings->SetGameData('BountygoldPMTotalWin', $totalWin);
                    ...
                    else if( $postData['slotEvent'] == 'gamble5GetDealerCard' ) 
                    {
                        ...
                        $slotSettings->SetGameData('BountygoldPMDealerCard', $tmpDc);
                    ...
                    {
                        $bstate = $slotSettings->GetGameData('BountygoldPMBonusState');
                        $Balance = $slotSettings->GetBalance();
                        if( $bstate == 0 ) 
                        {
                            $andFree = [
                            ...
                            $andFree[$postData['ind']] = $slotSettings->GetGameData('BountygoldPMFreeGames');
                            $stat[$postData['ind']] = 1;
                        ...
                        else if( $bstate == 1 ) 
                        {
                            ...
                            $andFree[$postData['ind']] = $slotSettings->GetGameData('BountygoldPMBonusMpl');
                            $stat[$postData['ind']] = 2;
                            $response = 'fsmul=' . $slotSettings->GetGameData('BountygoldPMBonusMpl') . '&bgid=0&balance=' . $Balance . '&wins=' . implode(',', $andFree) . '&fsmax=' . $slotSettings->GetGameData('BountygoldPMFreeGames') . '&level=2&index=149&balance_cash=' . $Balance . '&balance_bonus=0.00&na=s&fswin=0.00&status=' . implode(',', $stat) . '&stime=' . floor(microtime(true) * 1000) . '&fs=1&bgt=28&wins_mask=m,m,m,m,m,m&end=1&fsres=0.00&sver=5&n_reel_set=1&counter=298';
                            $bstate++;
                        }
                        $slotSettings->SetGameData('BountygoldPMBonusState', $bstate);
                    }
                    ...
                    {
                        if( $slotSettings->GetGameData('BountygoldPMCurrentFreeGame') < $slotSettings->GetGameData('BountygoldPMFreeGames') && $slotSettings->GetGameData('BountygoldPMFreeGames') > 0 ) 
                        {
                        ...
                        {
                            $slotSettings->SetGameData('BountygoldPMCurrentFreeGame', $slotSettings->GetGameData('BountygoldPMCurrentFreeGame') + 1);
                            $bonusMpl = $slotSettings->GetGameData('BountygoldPMBonusMpl');
                        }
                        $Balance = $slotSettings->GetBalance();
                        if( $postData['slotEvent'] != 'freespin' ) 
                        {
                        ...
                        if( $scattersCount >= 3 ) 
                        {
                            if( $slotSettings->GetGameData('BountygoldPMFreeGames') == 0 ) 
                            {
                                $slotSettings->SetGameData('BountygoldPMBonusMpl', rand(1, 18));
                                $slotSettings->SetGameData('BountygoldPMFreeGames', rand(10, 38));
                                $spinType = 'b&bw=1&bgid=0&bgt=28';
                            }
                            else
                            {
                                $slotSettings->SetGameData('BountygoldPMFreeGames', $slotSettings->GetGameData('BountygoldPMFreeGames') + $slotSettings->slotFreeCount);
                            }
                        }
                        if( $postData['slotEvent'] == 'freespin' ) 
                        {
                            $slotSettings->SetGameData('BountygoldPMBonusWin', $slotSettings->GetGameData('BountygoldPMBonusWin') + $totalWin);
                            $slotSettings->SetGameData('BountygoldPMTotalWin', $slotSettings->GetGameData('BountygoldPMTotalWin') + $totalWin);
                            $spinType = 's';
                            $Balance = $slotSettings->GetGameData('BountygoldPMFreeBalance');
                            if( $slotSettings->GetGameData('BountygoldPMFreeGames') <= $slotSettings->GetGameData('BountygoldPMCurrentFreeGame') && $slotSettings->GetGameData('BountygoldPMFreeGames') > 0 ) 
                            {
                                $spinType = 'c';
                                $winString .= ('&fs_total=' . $slotSettings->GetGameData('BountygoldPMFreeGames') . '&fsmul_total=' . $slotSettings->GetGameData('BountygoldPMBonusMpl') . '&fswin_total=' . $slotSettings->GetGameData('BountygoldPMBonusWin') . '&fsres_total=' . $slotSettings->GetGameData('BountygoldPMBonusWin') . '');
                            }
                            else
                            {
                                $winString .= ('&fsmul=' . $slotSettings->GetGameData('BountygoldPMBonusMpl') . '&fsmax=' . $slotSettings->GetGameData('BountygoldPMFreeGames') . '&fswin=' . $slotSettings->GetGameData('BountygoldPMTotalWin') . '&fs=' . $slotSettings->GetGameData('BountygoldPMCurrentFreeGame') . '&fsres=' . $slotSettings->GetGameData('BountygoldPMBonusWin'));
                            }
                        ...
                        else
                        {
                            ...
                            $slotSettings->SetGameData('BountygoldPMTotalWin', $totalWin);
                            $slotSettings->SetGameData('BountygoldPMBonusWin', $totalWin);
                        }
                        ...
                        $response = 'tw=' . $slotSettings->GetGameData('BountygoldPMBonusWin') . '&bstate=' . $slotSettings->GetGameData('BountygoldPMBonusState') . '&balance=' . $Balance . '&index=' . $postData['index'] . '&balance_cash=' . $Balance . '&balance_bonus=0.00' . $winString . '&na=' . $spinType . '&stime=' . floor(microtime(true) * 1000) . '&sa=' . $reels['reel1'][3] . ',' . $reels['reel2'][3] . ',' . $reels['reel3'][3] . ',' . $reels['reel4'][3] . ',' . $reels['reel5'][3] . '&sb=' . $reels['reel1'][-1] . ',' . $reels['reel2'][-1] . ',' . $reels['reel3'][-1] . ',' . $reels['reel4'][-1] . ',' . $reels['reel5'][-1] . '&sh=3&c=0.01&sver=5&n_reel_set=0&counter=' . ((int)$postData['counter'] + 1) . '&l=25&s=' . $s . '&w=' . $totalWinRaw . '';
                        if( $slotSettings->GetGameData('BountygoldPMFreeGames') <= $slotSettings->GetGameData('BountygoldPMCurrentFreeGame') && $slotSettings->GetGameData('BountygoldPMFreeGames') > 0 ) 
                        {
                            $slotSettings->SetGameData('BountygoldPMTotalWin', 0);
                            $slotSettings->SetGameData('BountygoldPMBonusWin', 0);
                        }
                        $response_log = '{"responseEvent":"spin","responseType":"' . $postData['slotEvent'] . '","serverResponse":{"BonusMpl":' . $slotSettings->GetGameData('BountygoldPMBonusMpl') . ',"lines":' . $postData['slotLines'] . ',"bet":' . $postData['slotBet'] . ',"totalFreeGames":' . $slotSettings->GetGameData('BountygoldPMFreeGames') . ',"currentFreeGames":' . $slotSettings->GetGameData('BountygoldPMCurrentFreeGame') . ',"Balance":' . $Balance . ',"afterBalance":' . $slotSettings->GetBalance() . ',"totalWin":' . $totalWin . ',"bonusWin":' . $slotSettings->GetGameData('BountygoldPMBonusWin') . ',"winLines":[],"Jackpots":' . $jsJack . ',"reelsSymbols":' . $jsSpin . '}}';
                        ...
                        if( $scattersCount >= 3 ) 
                        {
                            ...
                            {
                                $slotSettings->SetGameData('BountygoldPMFreeBalance', $Balance);
                                $slotSettings->SetGameData('BountygoldPMTotalWin', 0);
                                $slotSettings->SetGameData('BountygoldPMBonusState', 0);
                                $slotSettings->SetGameData('BountygoldPMBonusWin', $totalWin);
                            }
