            ...
            \DB::transaction(function() use ($request, $game)
            {
                try
                {
                    ...
                    if( $postData['slotEvent'] == 'doInit' ) 
                    {
                        ...
                        $fsInfo = '';
                        $slotSettings->SetGameData('JuicyFruitsPMBonusWin', 0);
                        $slotSettings->SetGameData('JuicyFruitsPMFreeGames', 0);
                        $slotSettings->SetGameData('JuicyFruitsPMCurrentFreeGame', 0);
                        $slotSettings->SetGameData('JuicyFruitsPMTotalWin', 0);
                        $slotSettings->SetGameData($slotSettings->slotId . 'FreeBalance', $slotSettings->GetBalance());
                        $slotSettings->SetGameData('JuicyFruitsPMBonusState', 0);
                        $slotSettings->SetGameData('JuicyFruitsPMBonusMpl', 0);
                        if( $lastEvent != 'NULL' ) 
                        {
                    ...
                    else if( $postData['slotEvent'] == 'gamble5GetUserCards' ) 
                    {
                        ...
                        $dealerCard = $slotSettings->GetGameData('JuicyFruitsPMDealerCard');
                        $totalWin = $slotSettings->GetGameData('JuicyFruitsPMTotalWin');
                        $gambleWin = 0;
                        ...
                        $slotSettings->SetGameData('JuicyFruitsPMTotalWin', $totalWin);
                    ...
                    else if( $postData['slotEvent'] == 'gamble5GetDealerCard' ) 
                    {
                        ...
                        $slotSettings->SetGameData('JuicyFruitsPMDealerCard', $tmpDc);
                    ...
                    {
                        $bstate = $slotSettings->GetGameData('JuicyFruitsPMBonusState');
                        $Balance = $slotSettings->GetBalance();
                        if( $bstate == 0 ) 
                        {
                            $andFree = [
                            ...
                            $andFree[$postData['ind']] = $slotSettings->GetGameData('JuicyFruitsPMFreeGames');
                            $stat[$postData['ind']] = 1;
                        ...
                        else if( $bstate == 1 ) 
                        {
                            ...
                            $andFree[$postData['ind']] = $slotSettings->GetGameData('JuicyFruitsPMBonusMpl');
                            $stat[$postData['ind']] = 2;
                            $response = 'fsmul=' . $slotSettings->GetGameData('JuicyFruitsPMBonusMpl') . '&bgid=0&balance=' . $Balance . '&wins=' . implode(',', $andFree) . '&fsmax=' . $slotSettings->GetGameData('JuicyFruitsPMFreeGames') . '&level=2&index=149&balance_cash=' . $Balance . '&balance_bonus=0.00&na=s&fswin=0.00&status=' . implode(',', $stat) . '&stime=' . floor(microtime(true) * 1000) . '&fs=1&bgt=28&wins_mask=m,m,m,m,m,m&end=1&fsres=0.00&sver=5&n_reel_set=1&counter=298';
                            $bstate++;
                        }
                        $slotSettings->SetGameData('JuicyFruitsPMBonusState', $bstate);
                    }
                    ...
                    {
                        if( $slotSettings->GetGameData('JuicyFruitsPMCurrentFreeGame') < $slotSettings->GetGameData('JuicyFruitsPMFreeGames') && $slotSettings->GetGameData('JuicyFruitsPMFreeGames') > 0 ) 
                        {
                        ...
                        {
                            $slotSettings->SetGameData('JuicyFruitsPMCurrentFreeGame', $slotSettings->GetGameData('JuicyFruitsPMCurrentFreeGame') + 1);
                            $bonusMpl = $slotSettings->GetGameData('JuicyFruitsPMBonusMpl');
                        }
                        $Balance = $slotSettings->GetBalance();
                        if( $postData['slotEvent'] != 'freespin' ) 
                        {
                        ...
                        if( $scattersCount >= 3 ) 
                        {
                            if( $slotSettings->GetGameData('JuicyFruitsPMFreeGames') == 0 ) 
                            {
                                $slotSettings->SetGameData('JuicyFruitsPMBonusMpl', rand(1, 18));
                                $slotSettings->SetGameData('JuicyFruitsPMFreeGames', rand(10, 38));
                                $spinType = 'b&bw=1&bgid=0&bgt=28';
                            }
                            else
                            {
                                $slotSettings->SetGameData('JuicyFruitsPMFreeGames', $slotSettings->GetGameData('JuicyFruitsPMFreeGames') + $slotSettings->slotFreeCount);
                            }
                        }
                        if( $postData['slotEvent'] == 'freespin' ) 
                        {
                            $slotSettings->SetGameData('JuicyFruitsPMBonusWin', $slotSettings->GetGameData('JuicyFruitsPMBonusWin') + $totalWin);
                            $slotSettings->SetGameData('JuicyFruitsPMTotalWin', $slotSettings->GetGameData('JuicyFruitsPMTotalWin') + $totalWin);
                            $spinType = 's';
                            $Balance = $slotSettings->GetGameData('JuicyFruitsPMFreeBalance');
                            if( $slotSettings->GetGameData('JuicyFruitsPMFreeGames') <= $slotSettings->GetGameData('JuicyFruitsPMCurrentFreeGame') && $slotSettings->GetGameData('JuicyFruitsPMFreeGames') > 0 ) 
                            {
                                $spinType = 'c';
                                $winString .= ('&fs_total=' . $slotSettings->GetGameData('JuicyFruitsPMFreeGames') . '&fsmul_total=' . $slotSettings->GetGameData('JuicyFruitsPMBonusMpl') . '&fswin_total=' . $slotSettings->GetGameData('JuicyFruitsPMBonusWin') . '&fsres_total=' . $slotSettings->GetGameData('JuicyFruitsPMBonusWin') . '');
                            }
                            else
                            {
                                $winString .= ('&fsmul=' . $slotSettings->GetGameData('JuicyFruitsPMBonusMpl') . '&fsmax=' . $slotSettings->GetGameData('JuicyFruitsPMFreeGames') . '&fswin=' . $slotSettings->GetGameData('JuicyFruitsPMTotalWin') . '&fs=' . $slotSettings->GetGameData('JuicyFruitsPMCurrentFreeGame') . '&fsres=' . $slotSettings->GetGameData('JuicyFruitsPMBonusWin'));
                            }
                        ...
                        else
                        {
                            ...
                            $slotSettings->SetGameData('JuicyFruitsPMTotalWin', $totalWin);
                            $slotSettings->SetGameData('JuicyFruitsPMBonusWin', $totalWin);
                        }
                        ...
                        $response = 'tw=' . $slotSettings->GetGameData('JuicyFruitsPMBonusWin') . '&bstate=' . $slotSettings->GetGameData('JuicyFruitsPMBonusState') . '&balance=' . $Balance . '&index=' . $postData['index'] . '&balance_cash=' . $Balance . '&balance_bonus=0.00' . $winString . '&na=' . $spinType . '&stime=' . floor(microtime(true) * 1000) . '&sa=' . $reels['reel1'][3] . ',' . $reels['reel2'][3] . ',' . $reels['reel3'][3] . ',' . $reels['reel4'][3] . ',' . $reels['reel5'][3] . '&sb=' . $reels['reel1'][-1] . ',' . $reels['reel2'][-1] . ',' . $reels['reel3'][-1] . ',' . $reels['reel4'][-1] . ',' . $reels['reel5'][-1] . '&sh=3&c=0.01&sver=5&n_reel_set=0&counter=' . ((int)$postData['counter'] + 1) . '&l=25&s=' . $s . '&w=' . $totalWinRaw . '';
                        if( $slotSettings->GetGameData('JuicyFruitsPMFreeGames') <= $slotSettings->GetGameData('JuicyFruitsPMCurrentFreeGame') && $slotSettings->GetGameData('JuicyFruitsPMFreeGames') > 0 ) 
                        {
                            $slotSettings->SetGameData('JuicyFruitsPMTotalWin', 0);
                            $slotSettings->SetGameData('JuicyFruitsPMBonusWin', 0);
                        }
                        $response_log = '{"responseEvent":"spin","responseType":"' . $postData['slotEvent'] . '","serverResponse":{"BonusMpl":' . $slotSettings->GetGameData('JuicyFruitsPMBonusMpl') . ',"lines":' . $postData['slotLines'] . ',"bet":' . $postData['slotBet'] . ',"totalFreeGames":' . $slotSettings->GetGameData('JuicyFruitsPMFreeGames') . ',"currentFreeGames":' . $slotSettings->GetGameData('JuicyFruitsPMCurrentFreeGame') . ',"Balance":' . $Balance . ',"afterBalance":' . $slotSettings->GetBalance() . ',"totalWin":' . $totalWin . ',"bonusWin":' . $slotSettings->GetGameData('JuicyFruitsPMBonusWin') . ',"winLines":[],"Jackpots":' . $jsJack . ',"reelsSymbols":' . $jsSpin . '}}';
                        ...
                        if( $scattersCount >= 3 ) 
                        {
                            ...
                            {
                                $slotSettings->SetGameData('JuicyFruitsPMFreeBalance', $Balance);
                                $slotSettings->SetGameData('JuicyFruitsPMTotalWin', 0);
                                $slotSettings->SetGameData('JuicyFruitsPMBonusState', 0);
                                $slotSettings->SetGameData('JuicyFruitsPMBonusWin', $totalWin);
                            }
