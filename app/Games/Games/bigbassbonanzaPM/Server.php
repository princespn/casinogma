            ...
            \DB::transaction(function() use ($request, $game)
            {
                try
                {
                    ...
                    if( $postData['slotEvent'] == 'doInit' ) 
                    {
                        ...
                        $fsInfo = '';
                        $slotSettings->SetGameData('bigbassbonanzaPMBonusWin', 0);
                        $slotSettings->SetGameData('bigbassbonanzaPMFreeGames', 0);
                        $slotSettings->SetGameData('bigbassbonanzaPMCurrentFreeGame', 0);
                        $slotSettings->SetGameData('bigbassbonanzaPMTotalWin', 0);
                        $slotSettings->SetGameData($slotSettings->slotId . 'FreeBalance', $slotSettings->GetBalance());
                        $slotSettings->SetGameData('bigbassbonanzaPMBonusState', 0);
                        $slotSettings->SetGameData('bigbassbonanzaPMBonusMpl', 0);
                        if( $lastEvent != 'NULL' ) 
                        {
                    ...
                    else if( $postData['slotEvent'] == 'gamble5GetUserCards' ) 
                    {
                        ...
                        $dealerCard = $slotSettings->GetGameData('bigbassbonanzaPMDealerCard');
                        $totalWin = $slotSettings->GetGameData('bigbassbonanzaPMTotalWin');
                        $gambleWin = 0;
                        ...
                        $slotSettings->SetGameData('bigbassbonanzaPMTotalWin', $totalWin);
                    ...
                    else if( $postData['slotEvent'] == 'gamble5GetDealerCard' ) 
                    {
                        ...
                        $slotSettings->SetGameData('bigbassbonanzaPMDealerCard', $tmpDc);
                    ...
                    {
                        $bstate = $slotSettings->GetGameData('bigbassbonanzaPMBonusState');
                        $Balance = $slotSettings->GetBalance();
                        if( $bstate == 0 ) 
                        {
                            $andFree = [
                            ...
                            $andFree[$postData['ind']] = $slotSettings->GetGameData('bigbassbonanzaPMFreeGames');
                            $stat[$postData['ind']] = 1;
                        ...
                        else if( $bstate == 1 ) 
                        {
                            ...
                            $andFree[$postData['ind']] = $slotSettings->GetGameData('bigbassbonanzaPMBonusMpl');
                            $stat[$postData['ind']] = 2;
                            $response = 'fsmul=' . $slotSettings->GetGameData('bigbassbonanzaPMBonusMpl') . '&bgid=0&balance=' . $Balance . '&wins=' . implode(',', $andFree) . '&fsmax=' . $slotSettings->GetGameData('bigbassbonanzaPMFreeGames') . '&level=2&index=149&balance_cash=' . $Balance . '&balance_bonus=0.00&na=s&fswin=0.00&status=' . implode(',', $stat) . '&stime=' . floor(microtime(true) * 1000) . '&fs=1&bgt=28&wins_mask=m,m,m,m,m,m&end=1&fsres=0.00&sver=5&n_reel_set=1&counter=298';
                            $bstate++;
                        }
                        $slotSettings->SetGameData('bigbassbonanzaPMBonusState', $bstate);
                    }
                    ...
                    {
                        if( $slotSettings->GetGameData('bigbassbonanzaPMCurrentFreeGame') < $slotSettings->GetGameData('bigbassbonanzaPMFreeGames') && $slotSettings->GetGameData('bigbassbonanzaPMFreeGames') > 0 ) 
                        {
                        ...
                        {
                            $slotSettings->SetGameData('bigbassbonanzaPMCurrentFreeGame', $slotSettings->GetGameData('bigbassbonanzaPMCurrentFreeGame') + 1);
                            $bonusMpl = $slotSettings->GetGameData('bigbassbonanzaPMBonusMpl');
                        }
                        $Balance = $slotSettings->GetBalance();
                        if( $postData['slotEvent'] != 'freespin' ) 
                        {
                        ...
                        if( $scattersCount >= 3 ) 
                        {
                            if( $slotSettings->GetGameData('bigbassbonanzaPMFreeGames') == 0 ) 
                            {
                                $slotSettings->SetGameData('bigbassbonanzaPMBonusMpl', rand(1, 18));
                                $slotSettings->SetGameData('bigbassbonanzaPMFreeGames', rand(10, 38));
                                $spinType = 'b&bw=1&bgid=0&bgt=28';
                            }
                            else
                            {
                                $slotSettings->SetGameData('bigbassbonanzaPMFreeGames', $slotSettings->GetGameData('bigbassbonanzaPMFreeGames') + $slotSettings->slotFreeCount);
                            }
                        }
                        if( $postData['slotEvent'] == 'freespin' ) 
                        {
                            $slotSettings->SetGameData('bigbassbonanzaPMBonusWin', $slotSettings->GetGameData('bigbassbonanzaPMBonusWin') + $totalWin);
                            $slotSettings->SetGameData('bigbassbonanzaPMTotalWin', $slotSettings->GetGameData('bigbassbonanzaPMTotalWin') + $totalWin);
                            $spinType = 's';
                            $Balance = $slotSettings->GetGameData('bigbassbonanzaPMFreeBalance');
                            if( $slotSettings->GetGameData('bigbassbonanzaPMFreeGames') <= $slotSettings->GetGameData('bigbassbonanzaPMCurrentFreeGame') && $slotSettings->GetGameData('bigbassbonanzaPMFreeGames') > 0 ) 
                            {
                                $spinType = 'c';
                                $winString .= ('&fs_total=' . $slotSettings->GetGameData('bigbassbonanzaPMFreeGames') . '&fsmul_total=' . $slotSettings->GetGameData('bigbassbonanzaPMBonusMpl') . '&fswin_total=' . $slotSettings->GetGameData('bigbassbonanzaPMBonusWin') . '&fsres_total=' . $slotSettings->GetGameData('bigbassbonanzaPMBonusWin') . '');
                            }
                            else
                            {
                                $winString .= ('&fsmul=' . $slotSettings->GetGameData('bigbassbonanzaPMBonusMpl') . '&fsmax=' . $slotSettings->GetGameData('bigbassbonanzaPMFreeGames') . '&fswin=' . $slotSettings->GetGameData('bigbassbonanzaPMTotalWin') . '&fs=' . $slotSettings->GetGameData('bigbassbonanzaPMCurrentFreeGame') . '&fsres=' . $slotSettings->GetGameData('bigbassbonanzaPMBonusWin'));
                            }
                        ...
                        else
                        {
                            ...
                            $slotSettings->SetGameData('bigbassbonanzaPMTotalWin', $totalWin);
                            $slotSettings->SetGameData('bigbassbonanzaPMBonusWin', $totalWin);
                        }
                        ...
                        $response = 'tw=' . $slotSettings->GetGameData('bigbassbonanzaPMBonusWin') . '&bstate=' . $slotSettings->GetGameData('bigbassbonanzaPMBonusState') . '&balance=' . $Balance . '&index=' . $postData['index'] . '&balance_cash=' . $Balance . '&balance_bonus=0.00' . $winString . '&na=' . $spinType . '&stime=' . floor(microtime(true) * 1000) . '&sa=' . $reels['reel1'][3] . ',' . $reels['reel2'][3] . ',' . $reels['reel3'][3] . ',' . $reels['reel4'][3] . ',' . $reels['reel5'][3] . '&sb=' . $reels['reel1'][-1] . ',' . $reels['reel2'][-1] . ',' . $reels['reel3'][-1] . ',' . $reels['reel4'][-1] . ',' . $reels['reel5'][-1] . '&sh=3&c=0.01&sver=5&n_reel_set=0&counter=' . ((int)$postData['counter'] + 1) . '&l=25&s=' . $s . '&w=' . $totalWinRaw . '';
                        if( $slotSettings->GetGameData('bigbassbonanzaPMFreeGames') <= $slotSettings->GetGameData('bigbassbonanzaPMCurrentFreeGame') && $slotSettings->GetGameData('bigbassbonanzaPMFreeGames') > 0 ) 
                        {
                            $slotSettings->SetGameData('bigbassbonanzaPMTotalWin', 0);
                            $slotSettings->SetGameData('bigbassbonanzaPMBonusWin', 0);
                        }
                        $response_log = '{"responseEvent":"spin","responseType":"' . $postData['slotEvent'] . '","serverResponse":{"BonusMpl":' . $slotSettings->GetGameData('bigbassbonanzaPMBonusMpl') . ',"lines":' . $postData['slotLines'] . ',"bet":' . $postData['slotBet'] . ',"totalFreeGames":' . $slotSettings->GetGameData('bigbassbonanzaPMFreeGames') . ',"currentFreeGames":' . $slotSettings->GetGameData('bigbassbonanzaPMCurrentFreeGame') . ',"Balance":' . $Balance . ',"afterBalance":' . $slotSettings->GetBalance() . ',"totalWin":' . $totalWin . ',"bonusWin":' . $slotSettings->GetGameData('bigbassbonanzaPMBonusWin') . ',"winLines":[],"Jackpots":' . $jsJack . ',"reelsSymbols":' . $jsSpin . '}}';
                        ...
                        if( $scattersCount >= 3 ) 
                        {
                            ...
                            {
                                $slotSettings->SetGameData('bigbassbonanzaPMFreeBalance', $Balance);
                                $slotSettings->SetGameData('bigbassbonanzaPMTotalWin', 0);
                                $slotSettings->SetGameData('bigbassbonanzaPMBonusState', 0);
                                $slotSettings->SetGameData('bigbassbonanzaPMBonusWin', $totalWin);
                            }
