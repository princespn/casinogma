            ...
            \DB::transaction(function() use ($request, $game)
            {
                try
                {
                    ...
                    if( $postData['slotEvent'] == 'doInit' ) 
                    {
                        ...
                        $fsInfo = '';
                        $slotSettings->SetGameData('AncientEgyptClassicBonusWin', 0);
                        $slotSettings->SetGameData('AncientEgyptClassicFreeGames', 0);
                        $slotSettings->SetGameData('AncientEgyptClassicCurrentFreeGame', 0);
                        $slotSettings->SetGameData('AncientEgyptClassicTotalWin', 0);
                        $slotSettings->SetGameData($slotSettings->slotId . 'FreeBalance', $slotSettings->GetBalance());
                        $slotSettings->SetGameData('AncientEgyptClassicBonusState', 0);
                        $slotSettings->SetGameData('AncientEgyptClassicBonusMpl', 0);
                        if( $lastEvent != 'NULL' ) 
                        {
                    ...
                    else if( $postData['slotEvent'] == 'gamble5GetUserCards' ) 
                    {
                        ...
                        $dealerCard = $slotSettings->GetGameData('AncientEgyptClassicDealerCard');
                        $totalWin = $slotSettings->GetGameData('AncientEgyptClassicTotalWin');
                        $gambleWin = 0;
                        ...
                        $slotSettings->SetGameData('AncientEgyptClassicTotalWin', $totalWin);
                    ...
                    else if( $postData['slotEvent'] == 'gamble5GetDealerCard' ) 
                    {
                        ...
                        $slotSettings->SetGameData('AncientEgyptClassicDealerCard', $tmpDc);
                    ...
                    {
                        $bstate = $slotSettings->GetGameData('AncientEgyptClassicBonusState');
                        $Balance = $slotSettings->GetBalance();
                        if( $bstate == 0 ) 
                        {
                            if( $postData['ind'] == 1 ) 
                            ...
                            $curWin = $slotSettings->GetGameData('AncientEgyptClassicBonusWin');
                            $curBet = $slotSettings->GetGameData('AncientEgyptClassicAllBet');
                        ...
                        $slotSettings->SetGameData('AncientEgyptClassicBonusState', $bstate);
                    }
                    else if( $postData['slotEvent'] == 'doMysteryScatter' ) 
                    {
                        $Balance = $slotSettings->GetBalance();
                        $rSym = rand(3, 11);
                        $slotSettings->SetGameData('AncientEgyptClassicFreeGames', $slotSettings->slotFreeCount);
                        $slotSettings->SetGameData('AncientEgyptClassicCurrentFreeGame', 0);
                        $slotSettings->SetGameData('AncientEgyptClassicSym', $rSym);
                    ...
                    {
                        if( $slotSettings->GetGameData('AncientEgyptClassicCurrentFreeGame') < $slotSettings->GetGameData('AncientEgyptClassicFreeGames') && $slotSettings->GetGameData('AncientEgyptClassicFreeGames') > 0 ) 
                        {
                        ...
                        if( $postData['slotEvent'] != 'freespin' ) 
                        {
                            ...
                            $slotSettings->SetGameData('AncientEgyptClassicBonusState', 0);
                            $slotSettings->SetGameData('AncientEgyptClassicBonusMpl', 0);
                            $slotSettings->SetGameData('AncientEgyptClassicSym', 0);
                            $slotSettings->SetGameData('AncientEgyptClassicAllBet', $postData['slotBet'] * $postData['slotLines']);
                        }
                        else
                        {
                            $slotSettings->SetGameData('AncientEgyptClassicCurrentFreeGame', $slotSettings->GetGameData('AncientEgyptClassicCurrentFreeGame') + 1);
                            $bonusMpl = $slotSettings->GetGameData('AncientEgyptClassicBonusMpl');
                        }
                        $Balance = $slotSettings->GetBalance();
                        if( $postData['slotEvent'] != 'freespin' ) 
                        {
                        ...
                        for( $i = 0; $i <= 2000; $i++ ) 
                        {
                            ...
                            for( $r = 1; $r <= 5; $r++ ) 
                            {
                                for( $p = 0; $p <= 2; $p++ ) 
                                {
                                    ...
                                    if( $reels['reel' . $r][$p] == $slotSettings->GetGameData('AncientEgyptClassicSym') ) 
                                    {
                                        $scattersCount2++;
                            ...
                            for( $r = 1; $r <= 5; $r++ ) 
                            {
                                for( $p = 0; $p <= 2; $p++ ) 
                                {
                                    if( $reels2['reel' . $r][$p] == $slotSettings->GetGameData('AncientEgyptClassicSym') ) 
                                    {
                                        $reels2['reel' . $r][0] = $slotSettings->GetGameData('AncientEgyptClassicSym');
                                        $reels2['reel' . $r][1] = $slotSettings->GetGameData('AncientEgyptClassicSym');
                                        $reels2['reel' . $r][2] = $slotSettings->GetGameData('AncientEgyptClassicSym');
                                        break;
                                    }
                                }
                            }
                            ...
                            if( isset($slotSettings->Paytable[$slotSettings->GetGameData('AncientEgyptClassicSym')]) && $slotSettings->Paytable[$slotSettings->GetGameData('AncientEgyptClassicSym')][$scattersCount2] > 0 ) 
                            {
                                $scattersWin2 = $slotSettings->Paytable['' . $slotSettings->GetGameData('AncientEgyptClassicSym')][$scattersCount2] * $postData['slotBet'] * $postData['slotLines'] * $bonusMpl;
                            }
                            else
                            {
                                $scattersWin2 = 0;
                            ...
                            if( $scattersWin2 > 0 ) 
                            {
                                $winString .= ('&psym=' . $slotSettings->GetGameData('AncientEgyptClassicSym') . '~' . $scattersWin . '~' . implode(',', $scattersStr));
                        ...
                        if( $scattersCount >= 3 ) 
                        {
                            if( $slotSettings->GetGameData('AncientEgyptClassicFreeGames') == 0 ) 
                            {
                                $slotSettings->SetGameData('AncientEgyptClassicBonusMpl', 1);
                                $spinType = 'b&status=0,0,0&rw=0.00&wins=0,0,0&bw=1&wins_mask=h,h,h&wp=0&end=0&bgt=28';
                            }
                            else
                            {
                                $slotSettings->SetGameData('AncientEgyptClassicFreeGames', $slotSettings->GetGameData('AncientEgyptClassicFreeGames') + $slotSettings->slotFreeCount);
                            }
                        }
                        if( $postData['slotEvent'] == 'freespin' ) 
                        {
                            $slotSettings->SetGameData('AncientEgyptClassicBonusWin', $slotSettings->GetGameData('AncientEgyptClassicBonusWin') + $totalWin);
                            $slotSettings->SetGameData('AncientEgyptClassicTotalWin', $slotSettings->GetGameData('AncientEgyptClassicTotalWin') + $totalWin);
                            $spinType = 's';
                            $Balance = $slotSettings->GetGameData('AncientEgyptClassicFreeBalance');
                            if( $slotSettings->GetGameData('AncientEgyptClassicFreeGames') <= $slotSettings->GetGameData('AncientEgyptClassicCurrentFreeGame') && $slotSettings->GetGameData('AncientEgyptClassicFreeGames') > 0 ) 
                            {
                                $spinType = 'c';
                                $winString .= ('&ms=' . $slotSettings->GetGameData('AncientEgyptClassicSym') . '&fs_total=' . $slotSettings->GetGameData('AncientEgyptClassicFreeGames') . '&fsmul_total=' . $slotSettings->GetGameData('AncientEgyptClassicBonusMpl') . '&fswin_total=' . $slotSettings->GetGameData('AncientEgyptClassicBonusWin') . '&fsres_total=' . $slotSettings->GetGameData('AncientEgyptClassicBonusWin') . '');
                            }
                            else
                            {
                                $winString .= ('&ms=' . $slotSettings->GetGameData('AncientEgyptClassicSym') . '&fsmul=' . $slotSettings->GetGameData('AncientEgyptClassicBonusMpl') . '&fsmax=' . $slotSettings->GetGameData('AncientEgyptClassicFreeGames') . '&fswin=' . $slotSettings->GetGameData('AncientEgyptClassicTotalWin') . '&fs=' . $slotSettings->GetGameData('AncientEgyptClassicCurrentFreeGame') . '&fsres=' . $slotSettings->GetGameData('AncientEgyptClassicBonusWin'));
                            }
                        ...
                        else
                        {
                            ...
                            $slotSettings->SetGameData('AncientEgyptClassicTotalWin', $totalWin);
                            $slotSettings->SetGameData('AncientEgyptClassicBonusWin', $totalWin);
                        }
                        ...
                        $response = 'tw=' . $slotSettings->GetGameData('AncientEgyptClassicBonusWin') . '&bstate=' . $slotSettings->GetGameData('AncientEgyptClassicBonusState') . '&balance=' . $Balance . '&index=' . $postData['index'] . '&balance_cash=' . $Balance . '&balance_bonus=0.00' . $winString . '&na=' . $spinType . '&stime=' . floor(microtime(true) * 1000) . '&sa=' . $reels['reel1'][3] . ',' . $reels['reel2'][3] . ',' . $reels['reel3'][3] . ',' . $reels['reel4'][3] . ',' . $reels['reel5'][3] . '&sb=' . $reels['reel1'][-1] . ',' . $reels['reel2'][-1] . ',' . $reels['reel3'][-1] . ',' . $reels['reel4'][-1] . ',' . $reels['reel5'][-1] . '&sh=3&c=0.01&sver=5&n_reel_set=0&counter=' . ((int)$postData['counter'] + 1) . '&l=10&s=' . $s . '&w=' . $totalWinRaw . '';
                        if( $slotSettings->GetGameData('AncientEgyptClassicFreeGames') <= $slotSettings->GetGameData('AncientEgyptClassicCurrentFreeGame') && $slotSettings->GetGameData('AncientEgyptClassicFreeGames') > 0 ) 
                        {
                            $slotSettings->SetGameData('AncientEgyptClassicTotalWin', 0);
                            $slotSettings->SetGameData('AncientEgyptClassicBonusWin', 0);
                        }
                        $response_log = '{"responseEvent":"spin","responseType":"' . $postData['slotEvent'] . '","serverResponse":{"Sym":' . $slotSettings->GetGameData('AncientEgyptClassicSym') . ',"BonusMpl":' . $slotSettings->GetGameData('AncientEgyptClassicBonusMpl') . ',"lines":' . $postData['slotLines'] . ',"bet":' . $postData['slotBet'] . ',"totalFreeGames":' . $slotSettings->GetGameData('AncientEgyptClassicFreeGames') . ',"currentFreeGames":' . $slotSettings->GetGameData('AncientEgyptClassicCurrentFreeGame') . ',"Balance":' . $Balance . ',"afterBalance":' . $slotSettings->GetBalance() . ',"totalWin":' . $totalWin . ',"bonusWin":' . $slotSettings->GetGameData('AncientEgyptClassicBonusWin') . ',"winLines":[],"Jackpots":' . $jsJack . ',"reelsSymbols":' . $jsSpin . '}}';
                        ...
                        if( $scattersCount >= 3 ) 
                        {
                            ...
                            {
                                $slotSettings->SetGameData('AncientEgyptClassicFreeBalance', $Balance);
                                $slotSettings->SetGameData('AncientEgyptClassicTotalWin', 0);
                                $slotSettings->SetGameData('AncientEgyptClassicBonusState', 0);
                                $slotSettings->SetGameData('AncientEgyptClassicBonusWin', $totalWin);
                            }
