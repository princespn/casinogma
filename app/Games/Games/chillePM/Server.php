<?php 
namespace VanguardLTE\Games\chillePM
{
    set_time_limit(5);
    class Server
    {
            ...
            \DB::transaction(function() use ($request, $game)
            {
                try
                {
                    ...
                    if( $postData['slotEvent'] == 'doInit' ) 
                    {
                        ...
                        $fsInfo = '';
                        $slotSettings->SetGameData('chillePMBonusWin', 0);
                        $slotSettings->SetGameData('chillePMFreeGames', 0);
                        $slotSettings->SetGameData('chillePMCurrentFreeGame', 0);
                        $slotSettings->SetGameData('chillePMTotalWin', 0);
                        $slotSettings->SetGameData($slotSettings->slotId . 'FreeBalance', $slotSettings->GetBalance());
                        $slotSettings->SetGameData('chillePMBonusState', 0);
                        $slotSettings->SetGameData('chillePMBonusMpl', 0);
                        if( $lastEvent != 'NULL' ) 
                        {
                    ...
                    else if( $postData['slotEvent'] == 'gamble5GetUserCards' ) 
                    {
                        ...
                        $dealerCard = $slotSettings->GetGameData('chillePMDealerCard');
                        $totalWin = $slotSettings->GetGameData('chillePMTotalWin');
                        $gambleWin = 0;
                        ...
                        $slotSettings->SetGameData('chillePMTotalWin', $totalWin);
                    ...
                    else if( $postData['slotEvent'] == 'gamble5GetDealerCard' ) 
                    {
                        ...
                        $slotSettings->SetGameData('chillePMDealerCard', $tmpDc);
                    ...
                    {
                        $bstate = $slotSettings->GetGameData('chillePMBonusState');
                        $Balance = $slotSettings->GetBalance();
                        if( $bstate == 0 ) 
                        {
                            $andFree = [
                            ...
                            $andFree[$postData['ind']] = $slotSettings->GetGameData('chillePMFreeGames');
                            $stat[$postData['ind']] = 1;
                        ...
                        else if( $bstate == 1 ) 
                        {
                            ...
                            $andFree[$postData['ind']] = $slotSettings->GetGameData('chillePMBonusMpl');
                            $stat[$postData['ind']] = 2;
                            $response = 'fsmul=' . $slotSettings->GetGameData('chillePMBonusMpl') . '&bgid=0&balance=' . $Balance . '&wins=' . implode(',', $andFree) . '&fsmax=' . $slotSettings->GetGameData('chillePMFreeGames') . '&level=2&index=149&balance_cash=' . $Balance . '&balance_bonus=0.00&na=s&fswin=0.00&status=' . implode(',', $stat) . '&stime=' . floor(microtime(true) * 1000) . '&fs=1&bgt=28&wins_mask=m,m,m,m,m,m&end=1&fsres=0.00&sver=5&n_reel_set=1&counter=298';
                            $bstate++;
                        }
                        $slotSettings->SetGameData('chillePMBonusState', $bstate);
                    }
                    ...
                    {
                        if( $slotSettings->GetGameData('chillePMCurrentFreeGame') < $slotSettings->GetGameData('chillePMFreeGames') && $slotSettings->GetGameData('chillePMFreeGames') > 0 ) 
                        {
                        ...
                        {
                            $slotSettings->SetGameData('chillePMCurrentFreeGame', $slotSettings->GetGameData('chillePMCurrentFreeGame') + 1);
                            $bonusMpl = $slotSettings->GetGameData('chillePMBonusMpl');
                        }
                        $Balance = $slotSettings->GetBalance();
                        if( $postData['slotEvent'] != 'freespin' ) 
                        {
                        ...
                        if( $scattersCount >= 3 ) 
                        {
                            if( $slotSettings->GetGameData('chillePMFreeGames') == 0 ) 
                            {
                                $slotSettings->SetGameData('chillePMBonusMpl', rand(1, 18));
                                $slotSettings->SetGameData('chillePMFreeGames', rand(10, 38));
                                $spinType = 'b&bw=1&bgid=0&bgt=28';
                            }
                            else
                            {
                                $slotSettings->SetGameData('chillePMFreeGames', $slotSettings->GetGameData('chillePMFreeGames') + $slotSettings->slotFreeCount);
                            }
                        }
                        if( $postData['slotEvent'] == 'freespin' ) 
                        {
                            $slotSettings->SetGameData('chillePMBonusWin', $slotSettings->GetGameData('chillePMBonusWin') + $totalWin);
                            $slotSettings->SetGameData('chillePMTotalWin', $slotSettings->GetGameData('chillePMTotalWin') + $totalWin);
                            $spinType = 's';
                            $Balance = $slotSettings->GetGameData('chillePMFreeBalance');
                            if( $slotSettings->GetGameData('chillePMFreeGames') <= $slotSettings->GetGameData('chillePMCurrentFreeGame') && $slotSettings->GetGameData('chillePMFreeGames') > 0 ) 
                            {
                                $spinType = 'c';
                                $winString .= ('&fs_total=' . $slotSettings->GetGameData('chillePMFreeGames') . '&fsmul_total=' . $slotSettings->GetGameData('chillePMBonusMpl') . '&fswin_total=' . $slotSettings->GetGameData('chillePMBonusWin') . '&fsres_total=' . $slotSettings->GetGameData('chillePMBonusWin') . '');
                            }
                            else
                            {
                                $winString .= ('&fsmul=' . $slotSettings->GetGameData('chillePMBonusMpl') . '&fsmax=' . $slotSettings->GetGameData('chillePMFreeGames') . '&fswin=' . $slotSettings->GetGameData('chillePMTotalWin') . '&fs=' . $slotSettings->GetGameData('chillePMCurrentFreeGame') . '&fsres=' . $slotSettings->GetGameData('chillePMBonusWin'));
                            }
                        ...
                        else
                        {
                            ...
                            $slotSettings->SetGameData('chillePMTotalWin', $totalWin);
                            $slotSettings->SetGameData('chillePMBonusWin', $totalWin);
                        }
                        ...
                        $response = 'tw=' . $slotSettings->GetGameData('chillePMBonusWin') . '&bstate=' . $slotSettings->GetGameData('chillePMBonusState') . '&balance=' . $Balance . '&index=' . $postData['index'] . '&balance_cash=' . $Balance . '&balance_bonus=0.00' . $winString . '&na=' . $spinType . '&stime=' . floor(microtime(true) * 1000) . '&sa=' . $reels['reel1'][3] . ',' . $reels['reel2'][3] . ',' . $reels['reel3'][3] . ',' . $reels['reel4'][3] . ',' . $reels['reel5'][3] . '&sb=' . $reels['reel1'][-1] . ',' . $reels['reel2'][-1] . ',' . $reels['reel3'][-1] . ',' . $reels['reel4'][-1] . ',' . $reels['reel5'][-1] . '&sh=3&c=0.01&sver=5&n_reel_set=0&counter=' . ((int)$postData['counter'] + 1) . '&l=25&s=' . $s . '&w=' . $totalWinRaw . '';
                        if( $slotSettings->GetGameData('chillePMFreeGames') <= $slotSettings->GetGameData('chillePMCurrentFreeGame') && $slotSettings->GetGameData('chillePMFreeGames') > 0 ) 
                        {
                            $slotSettings->SetGameData('chillePMTotalWin', 0);
                            $slotSettings->SetGameData('chillePMBonusWin', 0);
                        }
                        $response_log = '{"responseEvent":"spin","responseType":"' . $postData['slotEvent'] . '","serverResponse":{"BonusMpl":' . $slotSettings->GetGameData('chillePMBonusMpl') . ',"lines":' . $postData['slotLines'] . ',"bet":' . $postData['slotBet'] . ',"totalFreeGames":' . $slotSettings->GetGameData('chillePMFreeGames') . ',"currentFreeGames":' . $slotSettings->GetGameData('chillePMCurrentFreeGame') . ',"Balance":' . $Balance . ',"afterBalance":' . $slotSettings->GetBalance() . ',"totalWin":' . $totalWin . ',"bonusWin":' . $slotSettings->GetGameData('chillePMBonusWin') . ',"winLines":[],"Jackpots":' . $jsJack . ',"reelsSymbols":' . $jsSpin . '}}';
                        ...
                        if( $scattersCount >= 3 ) 
                        {
                            ...
                            {
                                $slotSettings->SetGameData('chillePMFreeBalance', $Balance);
                                $slotSettings->SetGameData('chillePMTotalWin', 0);
                                $slotSettings->SetGameData('chillePMBonusState', 0);
                                $slotSettings->SetGameData('chillePMBonusWin', $totalWin);
                            }
