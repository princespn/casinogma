            ...
            \DB::transaction(function() use ($request, $game)
            {
                try
                {
                    ...
                    if( $postData['slotEvent'] == 'doInit' ) 
                    {
                        ...
                        $fsInfo = '';
                        $slotSettings->SetGameData('AncientEgyptBonusWin', 0);
                        $slotSettings->SetGameData('AncientEgyptFreeGames', 0);
                        $slotSettings->SetGameData('AncientEgyptCurrentFreeGame', 0);
                        $slotSettings->SetGameData('AncientEgyptTotalWin', 0);
                        $slotSettings->SetGameData($slotSettings->slotId . 'FreeBalance', $slotSettings->GetBalance());
                        $slotSettings->SetGameData('AncientEgyptBonusState', 0);
                        $slotSettings->SetGameData('AncientEgyptBonusMpl', 0);
                        if( $lastEvent != 'NULL' ) 
                        {
                    ...
                    else if( $postData['slotEvent'] == 'gamble5GetUserCards' ) 
                    {
                        ...
                        $dealerCard = $slotSettings->GetGameData('AncientEgyptDealerCard');
                        $totalWin = $slotSettings->GetGameData('AncientEgyptTotalWin');
                        $gambleWin = 0;
                        ...
                        $slotSettings->SetGameData('AncientEgyptTotalWin', $totalWin);
                    ...
                    else if( $postData['slotEvent'] == 'gamble5GetDealerCard' ) 
                    {
                        ...
                        $slotSettings->SetGameData('AncientEgyptDealerCard', $tmpDc);
                    ...
                    {
                        $bstate = $slotSettings->GetGameData('AncientEgyptBonusState');
                        $Balance = $slotSettings->GetBalance();
                        if( $bstate == 0 ) 
                        {
                            if( $postData['ind'] == 1 ) 
                            ...
                            $curWin = $slotSettings->GetGameData('AncientEgyptBonusWin');
                            $curBet = $slotSettings->GetGameData('AncientEgyptAllBet');
                        ...
                        $slotSettings->SetGameData('AncientEgyptBonusState', $bstate);
                    }
                    else if( $postData['slotEvent'] == 'doMysteryScatter' ) 
                    {
                        $Balance = $slotSettings->GetBalance();
                        $rSym = rand(3, 11);
                        $slotSettings->SetGameData('AncientEgyptFreeGames', $slotSettings->slotFreeCount);
                        $slotSettings->SetGameData('AncientEgyptCurrentFreeGame', 0);
                        $slotSettings->SetGameData('AncientEgyptSym', $rSym);
                    ...
                    {
                        if( $slotSettings->GetGameData('AncientEgyptCurrentFreeGame') < $slotSettings->GetGameData('AncientEgyptFreeGames') && $slotSettings->GetGameData('AncientEgyptFreeGames') > 0 ) 
                        {
                        ...
                        if( $postData['slotEvent'] != 'freespin' ) 
                        {
                            ...
                            $slotSettings->SetGameData('AncientEgyptBonusState', 0);
                            $slotSettings->SetGameData('AncientEgyptBonusMpl', 0);
                            $slotSettings->SetGameData('AncientEgyptSym', 0);
                            $slotSettings->SetGameData('AncientEgyptAllBet', $postData['slotBet'] * $postData['slotLines']);
                        }
                        else
                        {
                            $slotSettings->SetGameData('AncientEgyptCurrentFreeGame', $slotSettings->GetGameData('AncientEgyptCurrentFreeGame') + 1);
                            $bonusMpl = $slotSettings->GetGameData('AncientEgyptBonusMpl');
                        }
                        $Balance = $slotSettings->GetBalance();
                        if( $postData['slotEvent'] != 'freespin' ) 
                        {
                        ...
                        for( $i = 0; $i <= 2000; $i++ ) 
                        {
                            ...
                            for( $r = 1; $r <= 5; $r++ ) 
                            {
                                for( $p = 0; $p <= 2; $p++ ) 
                                {
                                    ...
                                    if( $reels['reel' . $r][$p] == $slotSettings->GetGameData('AncientEgyptSym') ) 
                                    {
                                        $scattersCount2++;
                            ...
                            for( $r = 1; $r <= 5; $r++ ) 
                            {
                                for( $p = 0; $p <= 2; $p++ ) 
                                {
                                    if( $reels2['reel' . $r][$p] == $slotSettings->GetGameData('AncientEgyptSym') ) 
                                    {
                                        $reels2['reel' . $r][0] = $slotSettings->GetGameData('AncientEgyptSym');
                                        $reels2['reel' . $r][1] = $slotSettings->GetGameData('AncientEgyptSym');
                                        $reels2['reel' . $r][2] = $slotSettings->GetGameData('AncientEgyptSym');
                                        break;
                                    }
                                }
                            }
                            ...
                            if( isset($slotSettings->Paytable[$slotSettings->GetGameData('AncientEgyptSym')]) && $slotSettings->Paytable[$slotSettings->GetGameData('AncientEgyptSym')][$scattersCount2] > 0 ) 
                            {
                                $scattersWin2 = $slotSettings->Paytable['' . $slotSettings->GetGameData('AncientEgyptSym')][$scattersCount2] * $postData['slotBet'] * $postData['slotLines'] * $bonusMpl;
                            }
                            else
                            {
                                $scattersWin2 = 0;
                            ...
                            if( $scattersWin2 > 0 ) 
                            {
                                $winString .= ('&psym=' . $slotSettings->GetGameData('AncientEgyptSym') . '~' . $scattersWin . '~' . implode(',', $scattersStr));
                        ...
                        if( $scattersCount >= 3 ) 
                        {
                            if( $slotSettings->GetGameData('AncientEgyptFreeGames') == 0 ) 
                            {
                                $slotSettings->SetGameData('AncientEgyptBonusMpl', 1);
                                $spinType = 'b&status=0,0,0&rw=0.00&wins=0,0,0&bw=1&wins_mask=h,h,h&wp=0&end=0&bgt=28';
                            }
                            else
                            {
                                $slotSettings->SetGameData('AncientEgyptFreeGames', $slotSettings->GetGameData('AncientEgyptFreeGames') + $slotSettings->slotFreeCount);
                            }
                        }
                        if( $postData['slotEvent'] == 'freespin' ) 
                        {
                            $slotSettings->SetGameData('AncientEgyptBonusWin', $slotSettings->GetGameData('AncientEgyptBonusWin') + $totalWin);
                            $slotSettings->SetGameData('AncientEgyptTotalWin', $slotSettings->GetGameData('AncientEgyptTotalWin') + $totalWin);
                            $spinType = 's';
                            $Balance = $slotSettings->GetGameData('AncientEgyptFreeBalance');
                            if( $slotSettings->GetGameData('AncientEgyptFreeGames') <= $slotSettings->GetGameData('AncientEgyptCurrentFreeGame') && $slotSettings->GetGameData('AncientEgyptFreeGames') > 0 ) 
                            {
                                $spinType = 'c';
                                $winString .= ('&ms=' . $slotSettings->GetGameData('AncientEgyptSym') . '&fs_total=' . $slotSettings->GetGameData('AncientEgyptFreeGames') . '&fsmul_total=' . $slotSettings->GetGameData('AncientEgyptBonusMpl') . '&fswin_total=' . $slotSettings->GetGameData('AncientEgyptBonusWin') . '&fsres_total=' . $slotSettings->GetGameData('AncientEgyptBonusWin') . '');
                            }
                            else
                            {
                                $winString .= ('&ms=' . $slotSettings->GetGameData('AncientEgyptSym') . '&fsmul=' . $slotSettings->GetGameData('AncientEgyptBonusMpl') . '&fsmax=' . $slotSettings->GetGameData('AncientEgyptFreeGames') . '&fswin=' . $slotSettings->GetGameData('AncientEgyptTotalWin') . '&fs=' . $slotSettings->GetGameData('AncientEgyptCurrentFreeGame') . '&fsres=' . $slotSettings->GetGameData('AncientEgyptBonusWin'));
                            }
                        ...
                        else
                        {
                            ...
                            $slotSettings->SetGameData('AncientEgyptTotalWin', $totalWin);
                            $slotSettings->SetGameData('AncientEgyptBonusWin', $totalWin);
                        }
                        ...
                        $response = 'tw=' . $slotSettings->GetGameData('AncientEgyptBonusWin') . '&bstate=' . $slotSettings->GetGameData('AncientEgyptBonusState') . '&balance=' . $Balance . '&index=' . $postData['index'] . '&balance_cash=' . $Balance . '&balance_bonus=0.00' . $winString . '&na=' . $spinType . '&stime=' . floor(microtime(true) * 1000) . '&sa=' . $reels['reel1'][3] . ',' . $reels['reel2'][3] . ',' . $reels['reel3'][3] . ',' . $reels['reel4'][3] . ',' . $reels['reel5'][3] . '&sb=' . $reels['reel1'][-1] . ',' . $reels['reel2'][-1] . ',' . $reels['reel3'][-1] . ',' . $reels['reel4'][-1] . ',' . $reels['reel5'][-1] . '&sh=3&c=0.01&sver=5&n_reel_set=0&counter=' . ((int)$postData['counter'] + 1) . '&l=10&s=' . $s . '&w=' . $totalWinRaw . '';
                        if( $slotSettings->GetGameData('AncientEgyptFreeGames') <= $slotSettings->GetGameData('AncientEgyptCurrentFreeGame') && $slotSettings->GetGameData('AncientEgyptFreeGames') > 0 ) 
                        {
                            $slotSettings->SetGameData('AncientEgyptTotalWin', 0);
                            $slotSettings->SetGameData('AncientEgyptBonusWin', 0);
                        }
                        $response_log = '{"responseEvent":"spin","responseType":"' . $postData['slotEvent'] . '","serverResponse":{"Sym":' . $slotSettings->GetGameData('AncientEgyptSym') . ',"BonusMpl":' . $slotSettings->GetGameData('AncientEgyptBonusMpl') . ',"lines":' . $postData['slotLines'] . ',"bet":' . $postData['slotBet'] . ',"totalFreeGames":' . $slotSettings->GetGameData('AncientEgyptFreeGames') . ',"currentFreeGames":' . $slotSettings->GetGameData('AncientEgyptCurrentFreeGame') . ',"Balance":' . $Balance . ',"afterBalance":' . $slotSettings->GetBalance() . ',"totalWin":' . $totalWin . ',"bonusWin":' . $slotSettings->GetGameData('AncientEgyptBonusWin') . ',"winLines":[],"Jackpots":' . $jsJack . ',"reelsSymbols":' . $jsSpin . '}}';
                        ...
                        if( $scattersCount >= 3 ) 
                        {
                            ...
                            {
                                $slotSettings->SetGameData('AncientEgyptFreeBalance', $Balance);
                                $slotSettings->SetGameData('AncientEgyptTotalWin', 0);
                                $slotSettings->SetGameData('AncientEgyptBonusState', 0);
                                $slotSettings->SetGameData('AncientEgyptBonusWin', $totalWin);
                            }
