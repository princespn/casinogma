<?php 
namespace VanguardLTE\Games\waysmadamePM
{
    set_time_limit(5);
    class Server
    {
            ...
            \DB::transaction(function() use ($request, $game)
            {
                try
                {
                    ...
                    if( $postData['slotEvent'] == 'doInit' ) 
                    {
                        ...
                        $fsInfo = '';
                        $slotSettings->SetGameData('waysmadamePMBonusWin', 0);
                        $slotSettings->SetGameData('waysmadamePMFreeGames', 0);
                        $slotSettings->SetGameData('waysmadamePMCurrentFreeGame', 0);
                        $slotSettings->SetGameData('waysmadamePMTotalWin', 0);
                        $slotSettings->SetGameData($slotSettings->slotId . 'FreeBalance', $slotSettings->GetBalance());
                        $slotSettings->SetGameData('waysmadamePMBonusState', 0);
                        $slotSettings->SetGameData('waysmadamePMBonusMpl', 0);
                        if( $lastEvent != 'NULL' ) 
                        {
                    ...
                    else if( $postData['slotEvent'] == 'gamble5GetUserCards' ) 
                    {
                        ...
                        $dealerCard = $slotSettings->GetGameData('waysmadamePMDealerCard');
                        $totalWin = $slotSettings->GetGameData('waysmadamePMTotalWin');
                        $gambleWin = 0;
                        ...
                        $slotSettings->SetGameData('waysmadamePMTotalWin', $totalWin);
                    ...
                    else if( $postData['slotEvent'] == 'gamble5GetDealerCard' ) 
                    {
                        ...
                        $slotSettings->SetGameData('waysmadamePMDealerCard', $tmpDc);
                    ...
                    {
                        $bstate = $slotSettings->GetGameData('waysmadamePMBonusState');
                        $Balance = $slotSettings->GetBalance();
                        if( $bstate == 0 ) 
                        {
                            $andFree = [
                            ...
                            $andFree[$postData['ind']] = $slotSettings->GetGameData('waysmadamePMFreeGames');
                            $stat[$postData['ind']] = 1;
                        ...
                        else if( $bstate == 1 ) 
                        {
                            ...
                            $andFree[$postData['ind']] = $slotSettings->GetGameData('waysmadamePMBonusMpl');
                            $stat[$postData['ind']] = 2;
                            $response = 'fsmul=' . $slotSettings->GetGameData('waysmadamePMBonusMpl') . '&bgid=0&balance=' . $Balance . '&wins=' . implode(',', $andFree) . '&fsmax=' . $slotSettings->GetGameData('waysmadamePMFreeGames') . '&level=2&index=149&balance_cash=' . $Balance . '&balance_bonus=0.00&na=s&fswin=0.00&status=' . implode(',', $stat) . '&stime=' . floor(microtime(true) * 1000) . '&fs=1&bgt=28&wins_mask=m,m,m,m,m,m&end=1&fsres=0.00&sver=5&n_reel_set=1&counter=298';
                            $bstate++;
                        }
                        $slotSettings->SetGameData('waysmadamePMBonusState', $bstate);
                    }
                    ...
                    {
                        if( $slotSettings->GetGameData('waysmadamePMCurrentFreeGame') < $slotSettings->GetGameData('waysmadamePMFreeGames') && $slotSettings->GetGameData('waysmadamePMFreeGames') > 0 ) 
                        {
                        ...
                        {
                            $slotSettings->SetGameData('waysmadamePMCurrentFreeGame', $slotSettings->GetGameData('waysmadamePMCurrentFreeGame') + 1);
                            $bonusMpl = $slotSettings->GetGameData('waysmadamePMBonusMpl');
                        }
                        $Balance = $slotSettings->GetBalance();
                        if( $postData['slotEvent'] != 'freespin' ) 
                        {
                        ...
                        if( $scattersCount >= 3 ) 
                        {
                            if( $slotSettings->GetGameData('waysmadamePMFreeGames') == 0 ) 
                            {
                                $slotSettings->SetGameData('waysmadamePMBonusMpl', rand(1, 18));
                                $slotSettings->SetGameData('waysmadamePMFreeGames', rand(10, 38));
                                $spinType = 'b&bw=1&bgid=0&bgt=28';
                            }
                            else
                            {
                                $slotSettings->SetGameData('waysmadamePMFreeGames', $slotSettings->GetGameData('waysmadamePMFreeGames') + $slotSettings->slotFreeCount);
                            }
                        }
                        if( $postData['slotEvent'] == 'freespin' ) 
                        {
                            $slotSettings->SetGameData('waysmadamePMBonusWin', $slotSettings->GetGameData('waysmadamePMBonusWin') + $totalWin);
                            $slotSettings->SetGameData('waysmadamePMTotalWin', $slotSettings->GetGameData('waysmadamePMTotalWin') + $totalWin);
                            $spinType = 's';
                            $Balance = $slotSettings->GetGameData('waysmadamePMFreeBalance');
                            if( $slotSettings->GetGameData('waysmadamePMFreeGames') <= $slotSettings->GetGameData('waysmadamePMCurrentFreeGame') && $slotSettings->GetGameData('waysmadamePMFreeGames') > 0 ) 
                            {
                                $spinType = 'c';
                                $winString .= ('&fs_total=' . $slotSettings->GetGameData('waysmadamePMFreeGames') . '&fsmul_total=' . $slotSettings->GetGameData('waysmadamePMBonusMpl') . '&fswin_total=' . $slotSettings->GetGameData('waysmadamePMBonusWin') . '&fsres_total=' . $slotSettings->GetGameData('waysmadamePMBonusWin') . '');
                            }
                            else
                            {
                                $winString .= ('&fsmul=' . $slotSettings->GetGameData('waysmadamePMBonusMpl') . '&fsmax=' . $slotSettings->GetGameData('waysmadamePMFreeGames') . '&fswin=' . $slotSettings->GetGameData('waysmadamePMTotalWin') . '&fs=' . $slotSettings->GetGameData('waysmadamePMCurrentFreeGame') . '&fsres=' . $slotSettings->GetGameData('waysmadamePMBonusWin'));
                            }
                        ...
                        else
                        {
                            ...
                            $slotSettings->SetGameData('waysmadamePMTotalWin', $totalWin);
                            $slotSettings->SetGameData('waysmadamePMBonusWin', $totalWin);
                        }
                        ...
                        $response = 'tw=' . $slotSettings->GetGameData('waysmadamePMBonusWin') . '&bstate=' . $slotSettings->GetGameData('waysmadamePMBonusState') . '&balance=' . $Balance . '&index=' . $postData['index'] . '&balance_cash=' . $Balance . '&balance_bonus=0.00' . $winString . '&na=' . $spinType . '&stime=' . floor(microtime(true) * 1000) . '&sa=' . $reels['reel1'][3] . ',' . $reels['reel2'][3] . ',' . $reels['reel3'][3] . ',' . $reels['reel4'][3] . ',' . $reels['reel5'][3] . '&sb=' . $reels['reel1'][-1] . ',' . $reels['reel2'][-1] . ',' . $reels['reel3'][-1] . ',' . $reels['reel4'][-1] . ',' . $reels['reel5'][-1] . '&sh=3&c=0.01&sver=5&n_reel_set=0&counter=' . ((int)$postData['counter'] + 1) . '&l=25&s=' . $s . '&w=' . $totalWinRaw . '';
                        if( $slotSettings->GetGameData('waysmadamePMFreeGames') <= $slotSettings->GetGameData('waysmadamePMCurrentFreeGame') && $slotSettings->GetGameData('waysmadamePMFreeGames') > 0 ) 
                        {
                            $slotSettings->SetGameData('waysmadamePMTotalWin', 0);
                            $slotSettings->SetGameData('waysmadamePMBonusWin', 0);
                        }
                        $response_log = '{"responseEvent":"spin","responseType":"' . $postData['slotEvent'] . '","serverResponse":{"BonusMpl":' . $slotSettings->GetGameData('waysmadamePMBonusMpl') . ',"lines":' . $postData['slotLines'] . ',"bet":' . $postData['slotBet'] . ',"totalFreeGames":' . $slotSettings->GetGameData('waysmadamePMFreeGames') . ',"currentFreeGames":' . $slotSettings->GetGameData('waysmadamePMCurrentFreeGame') . ',"Balance":' . $Balance . ',"afterBalance":' . $slotSettings->GetBalance() . ',"totalWin":' . $totalWin . ',"bonusWin":' . $slotSettings->GetGameData('waysmadamePMBonusWin') . ',"winLines":[],"Jackpots":' . $jsJack . ',"reelsSymbols":' . $jsSpin . '}}';
                        ...
                        if( $scattersCount >= 3 ) 
                        {
                            ...
                            {
                                $slotSettings->SetGameData('waysmadamePMFreeBalance', $Balance);
                                $slotSettings->SetGameData('waysmadamePMTotalWin', 0);
                                $slotSettings->SetGameData('waysmadamePMBonusState', 0);
                                $slotSettings->SetGameData('waysmadamePMBonusWin', $totalWin);
                            }
