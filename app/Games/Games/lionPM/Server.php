<?php 
namespace VanguardLTE\Games\lionPM
{
    set_time_limit(5);
    class Server
    {
            ...
            \DB::transaction(function() use ($request, $game)
            {
                try
                {
                    ...
                    if( $postData['slotEvent'] == 'doInit' ) 
                    {
                        ...
                        $fsInfo = '';
                        $slotSettings->SetGameData('lionPMBonusWin', 0);
                        $slotSettings->SetGameData('lionPMFreeGames', 0);
                        $slotSettings->SetGameData('lionPMCurrentFreeGame', 0);
                        $slotSettings->SetGameData('lionPMTotalWin', 0);
                        $slotSettings->SetGameData($slotSettings->slotId . 'FreeBalance', $slotSettings->GetBalance());
                        $slotSettings->SetGameData('lionPMBonusState', 0);
                        $slotSettings->SetGameData('lionPMBonusMpl', 0);
                        if( $lastEvent != 'NULL' ) 
                        {
                    ...
                    else if( $postData['slotEvent'] == 'gamble5GetUserCards' ) 
                    {
                        ...
                        $dealerCard = $slotSettings->GetGameData('lionPMDealerCard');
                        $totalWin = $slotSettings->GetGameData('lionPMTotalWin');
                        $gambleWin = 0;
                        ...
                        $slotSettings->SetGameData('lionPMTotalWin', $totalWin);
                    ...
                    else if( $postData['slotEvent'] == 'gamble5GetDealerCard' ) 
                    {
                        ...
                        $slotSettings->SetGameData('lionPMDealerCard', $tmpDc);
                    ...
                    {
                        $bstate = $slotSettings->GetGameData('lionPMBonusState');
                        $Balance = $slotSettings->GetBalance();
                        if( $bstate == 0 ) 
                        {
                            $andFree = [
                            ...
                            $andFree[$postData['ind']] = $slotSettings->GetGameData('lionPMFreeGames');
                            $stat[$postData['ind']] = 1;
                        ...
                        else if( $bstate == 1 ) 
                        {
                            ...
                            $andFree[$postData['ind']] = $slotSettings->GetGameData('lionPMBonusMpl');
                            $stat[$postData['ind']] = 2;
                            $response = 'fsmul=' . $slotSettings->GetGameData('lionPMBonusMpl') . '&bgid=0&balance=' . $Balance . '&wins=' . implode(',', $andFree) . '&fsmax=' . $slotSettings->GetGameData('lionPMFreeGames') . '&level=2&index=149&balance_cash=' . $Balance . '&balance_bonus=0.00&na=s&fswin=0.00&status=' . implode(',', $stat) . '&stime=' . floor(microtime(true) * 1000) . '&fs=1&bgt=28&wins_mask=m,m,m,m,m,m&end=1&fsres=0.00&sver=5&n_reel_set=1&counter=298';
                            $bstate++;
                        }
                        $slotSettings->SetGameData('lionPMBonusState', $bstate);
                    }
                    ...
                    {
                        if( $slotSettings->GetGameData('lionPMCurrentFreeGame') < $slotSettings->GetGameData('lionPMFreeGames') && $slotSettings->GetGameData('lionPMFreeGames') > 0 ) 
                        {
                        ...
                        {
                            $slotSettings->SetGameData('lionPMCurrentFreeGame', $slotSettings->GetGameData('lionPMCurrentFreeGame') + 1);
                            $bonusMpl = $slotSettings->GetGameData('lionPMBonusMpl');
                        }
                        $Balance = $slotSettings->GetBalance();
                        if( $postData['slotEvent'] != 'freespin' ) 
                        {
                        ...
                        if( $scattersCount >= 3 ) 
                        {
                            if( $slotSettings->GetGameData('lionPMFreeGames') == 0 ) 
                            {
                                $slotSettings->SetGameData('lionPMBonusMpl', rand(1, 18));
                                $slotSettings->SetGameData('lionPMFreeGames', rand(10, 38));
                                $spinType = 'b&bw=1&bgid=0&bgt=28';
                            }
                            else
                            {
                                $slotSettings->SetGameData('lionPMFreeGames', $slotSettings->GetGameData('lionPMFreeGames') + $slotSettings->slotFreeCount);
                            }
                        }
                        if( $postData['slotEvent'] == 'freespin' ) 
                        {
                            $slotSettings->SetGameData('lionPMBonusWin', $slotSettings->GetGameData('lionPMBonusWin') + $totalWin);
                            $slotSettings->SetGameData('lionPMTotalWin', $slotSettings->GetGameData('lionPMTotalWin') + $totalWin);
                            $spinType = 's';
                            $Balance = $slotSettings->GetGameData('lionPMFreeBalance');
                            if( $slotSettings->GetGameData('lionPMFreeGames') <= $slotSettings->GetGameData('lionPMCurrentFreeGame') && $slotSettings->GetGameData('lionPMFreeGames') > 0 ) 
                            {
                                $spinType = 'c';
                                $winString .= ('&fs_total=' . $slotSettings->GetGameData('lionPMFreeGames') . '&fsmul_total=' . $slotSettings->GetGameData('lionPMBonusMpl') . '&fswin_total=' . $slotSettings->GetGameData('lionPMBonusWin') . '&fsres_total=' . $slotSettings->GetGameData('lionPMBonusWin') . '');
                            }
                            else
                            {
                                $winString .= ('&fsmul=' . $slotSettings->GetGameData('lionPMBonusMpl') . '&fsmax=' . $slotSettings->GetGameData('lionPMFreeGames') . '&fswin=' . $slotSettings->GetGameData('lionPMTotalWin') . '&fs=' . $slotSettings->GetGameData('lionPMCurrentFreeGame') . '&fsres=' . $slotSettings->GetGameData('lionPMBonusWin'));
                            }
                        ...
                        else
                        {
                            ...
                            $slotSettings->SetGameData('lionPMTotalWin', $totalWin);
                            $slotSettings->SetGameData('lionPMBonusWin', $totalWin);
                        }
                        ...
                        $response = 'tw=' . $slotSettings->GetGameData('lionPMBonusWin') . '&bstate=' . $slotSettings->GetGameData('lionPMBonusState') . '&balance=' . $Balance . '&index=' . $postData['index'] . '&balance_cash=' . $Balance . '&balance_bonus=0.00' . $winString . '&na=' . $spinType . '&stime=' . floor(microtime(true) * 1000) . '&sa=' . $reels['reel1'][3] . ',' . $reels['reel2'][3] . ',' . $reels['reel3'][3] . ',' . $reels['reel4'][3] . ',' . $reels['reel5'][3] . '&sb=' . $reels['reel1'][-1] . ',' . $reels['reel2'][-1] . ',' . $reels['reel3'][-1] . ',' . $reels['reel4'][-1] . ',' . $reels['reel5'][-1] . '&sh=3&c=0.01&sver=5&n_reel_set=0&counter=' . ((int)$postData['counter'] + 1) . '&l=25&s=' . $s . '&w=' . $totalWinRaw . '';
                        if( $slotSettings->GetGameData('lionPMFreeGames') <= $slotSettings->GetGameData('lionPMCurrentFreeGame') && $slotSettings->GetGameData('lionPMFreeGames') > 0 ) 
                        {
                            $slotSettings->SetGameData('lionPMTotalWin', 0);
                            $slotSettings->SetGameData('lionPMBonusWin', 0);
                        }
                        $response_log = '{"responseEvent":"spin","responseType":"' . $postData['slotEvent'] . '","serverResponse":{"BonusMpl":' . $slotSettings->GetGameData('lionPMBonusMpl') . ',"lines":' . $postData['slotLines'] . ',"bet":' . $postData['slotBet'] . ',"totalFreeGames":' . $slotSettings->GetGameData('lionPMFreeGames') . ',"currentFreeGames":' . $slotSettings->GetGameData('lionPMCurrentFreeGame') . ',"Balance":' . $Balance . ',"afterBalance":' . $slotSettings->GetBalance() . ',"totalWin":' . $totalWin . ',"bonusWin":' . $slotSettings->GetGameData('lionPMBonusWin') . ',"winLines":[],"Jackpots":' . $jsJack . ',"reelsSymbols":' . $jsSpin . '}}';
                        ...
                        if( $scattersCount >= 3 ) 
                        {
                            ...
                            {
                                $slotSettings->SetGameData('lionPMFreeBalance', $Balance);
                                $slotSettings->SetGameData('lionPMTotalWin', 0);
                                $slotSettings->SetGameData('lionPMBonusState', 0);
                                $slotSettings->SetGameData('lionPMBonusWin', $totalWin);
                            }
