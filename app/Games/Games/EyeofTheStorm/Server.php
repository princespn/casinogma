            ...
            \DB::transaction(function() use ($request, $game)
            {
                try
                {
                    ...
                    if( $postData['slotEvent'] == 'doInit' ) 
                    {
                        ...
                        $fsInfo = '';
                        $slotSettings->SetGameData('EyeofTheStormBonusWin', 0);
                        $slotSettings->SetGameData('EyeofTheStormFreeGames', 0);
                        $slotSettings->SetGameData('EyeofTheStormCurrentFreeGame', 0);
                        $slotSettings->SetGameData('EyeofTheStormTotalWin', 0);
                        $slotSettings->SetGameData($slotSettings->slotId . 'FreeBalance', $slotSettings->GetBalance());
                        $slotSettings->SetGameData('EyeofTheStormBonusState', 0);
                        $slotSettings->SetGameData('EyeofTheStormBonusMpl', 0);
                        if( $lastEvent != 'NULL' ) 
                        {
                    ...
                    else if( $postData['slotEvent'] == 'gamble5GetUserCards' ) 
                    {
                        ...
                        $dealerCard = $slotSettings->GetGameData('EyeofTheStormDealerCard');
                        $totalWin = $slotSettings->GetGameData('EyeofTheStormTotalWin');
                        $gambleWin = 0;
                        ...
                        $slotSettings->SetGameData('EyeofTheStormTotalWin', $totalWin);
                    ...
                    else if( $postData['slotEvent'] == 'gamble5GetDealerCard' ) 
                    {
                        ...
                        $slotSettings->SetGameData('EyeofTheStormDealerCard', $tmpDc);
                    ...
                    {
                        $bstate = $slotSettings->GetGameData('EyeofTheStormBonusState');
                        $Balance = $slotSettings->GetBalance();
                        if( $bstate == 0 ) 
                        {
                            if( $postData['ind'] == 1 ) 
                            ...
                            $curWin = $slotSettings->GetGameData('EyeofTheStormBonusWin');
                            $curBet = $slotSettings->GetGameData('EyeofTheStormAllBet');
                        ...
                        $slotSettings->SetGameData('EyeofTheStormBonusState', $bstate);
                    }
                    else if( $postData['slotEvent'] == 'doMysteryScatter' ) 
                    {
                        $Balance = $slotSettings->GetBalance();
                        $rSym = rand(3, 11);
                        $slotSettings->SetGameData('EyeofTheStormFreeGames', $slotSettings->slotFreeCount);
                        $slotSettings->SetGameData('EyeofTheStormCurrentFreeGame', 0);
                        $slotSettings->SetGameData('EyeofTheStormSym', $rSym);
                    ...
                    {
                        if( $slotSettings->GetGameData('EyeofTheStormCurrentFreeGame') < $slotSettings->GetGameData('EyeofTheStormFreeGames') && $slotSettings->GetGameData('EyeofTheStormFreeGames') > 0 ) 
                        {
                        ...
                        if( $postData['slotEvent'] != 'freespin' ) 
                        {
                            ...
                            $slotSettings->SetGameData('EyeofTheStormBonusState', 0);
                            $slotSettings->SetGameData('EyeofTheStormBonusMpl', 0);
                            $slotSettings->SetGameData('EyeofTheStormSym', 0);
                            $slotSettings->SetGameData('EyeofTheStormAllBet', $postData['slotBet'] * $postData['slotLines']);
                        }
                        else
                        {
                            $slotSettings->SetGameData('EyeofTheStormCurrentFreeGame', $slotSettings->GetGameData('EyeofTheStormCurrentFreeGame') + 1);
                            $bonusMpl = $slotSettings->GetGameData('EyeofTheStormBonusMpl');
                        }
                        $Balance = $slotSettings->GetBalance();
                        if( $postData['slotEvent'] != 'freespin' ) 
                        {
                        ...
                        for( $i = 0; $i <= 2000; $i++ ) 
                        {
                            ...
                            for( $r = 1; $r <= 5; $r++ ) 
                            {
                                for( $p = 0; $p <= 2; $p++ ) 
                                {
                                    ...
                                    if( $reels['reel' . $r][$p] == $slotSettings->GetGameData('EyeofTheStormSym') ) 
                                    {
                                        $scattersCount2++;
                            ...
                            for( $r = 1; $r <= 5; $r++ ) 
                            {
                                for( $p = 0; $p <= 2; $p++ ) 
                                {
                                    if( $reels2['reel' . $r][$p] == $slotSettings->GetGameData('EyeofTheStormSym') ) 
                                    {
                                        $reels2['reel' . $r][0] = $slotSettings->GetGameData('EyeofTheStormSym');
                                        $reels2['reel' . $r][1] = $slotSettings->GetGameData('EyeofTheStormSym');
                                        $reels2['reel' . $r][2] = $slotSettings->GetGameData('EyeofTheStormSym');
                                        break;
                                    }
                                }
                            }
                            ...
                            if( isset($slotSettings->Paytable[$slotSettings->GetGameData('EyeofTheStormSym')]) && $slotSettings->Paytable[$slotSettings->GetGameData('EyeofTheStormSym')][$scattersCount2] > 0 ) 
                            {
                                $scattersWin2 = $slotSettings->Paytable['' . $slotSettings->GetGameData('EyeofTheStormSym')][$scattersCount2] * $postData['slotBet'] * $postData['slotLines'] * $bonusMpl;
                            }
                            else
                            {
                                $scattersWin2 = 0;
                            ...
                            if( $scattersWin2 > 0 ) 
                            {
                                $winString .= ('&psym=' . $slotSettings->GetGameData('EyeofTheStormSym') . '~' . $scattersWin . '~' . implode(',', $scattersStr));
                        ...
                        if( $scattersCount >= 3 ) 
                        {
                            if( $slotSettings->GetGameData('EyeofTheStormFreeGames') == 0 ) 
                            {
                                $slotSettings->SetGameData('EyeofTheStormBonusMpl', 1);
                                $spinType = 'b&status=0,0,0&rw=0.00&wins=0,0,0&bw=1&wins_mask=h,h,h&wp=0&end=0&bgt=28';
                            }
                            else
                            {
                                $slotSettings->SetGameData('EyeofTheStormFreeGames', $slotSettings->GetGameData('EyeofTheStormFreeGames') + $slotSettings->slotFreeCount);
                            }
                        }
                        if( $postData['slotEvent'] == 'freespin' ) 
                        {
                            $slotSettings->SetGameData('EyeofTheStormBonusWin', $slotSettings->GetGameData('EyeofTheStormBonusWin') + $totalWin);
                            $slotSettings->SetGameData('EyeofTheStormTotalWin', $slotSettings->GetGameData('EyeofTheStormTotalWin') + $totalWin);
                            $spinType = 's';
                            $Balance = $slotSettings->GetGameData('EyeofTheStormFreeBalance');
                            if( $slotSettings->GetGameData('EyeofTheStormFreeGames') <= $slotSettings->GetGameData('EyeofTheStormCurrentFreeGame') && $slotSettings->GetGameData('EyeofTheStormFreeGames') > 0 ) 
                            {
                                $spinType = 'c';
                                $winString .= ('&ms=' . $slotSettings->GetGameData('EyeofTheStormSym') . '&fs_total=' . $slotSettings->GetGameData('EyeofTheStormFreeGames') . '&fsmul_total=' . $slotSettings->GetGameData('EyeofTheStormBonusMpl') . '&fswin_total=' . $slotSettings->GetGameData('EyeofTheStormBonusWin') . '&fsres_total=' . $slotSettings->GetGameData('EyeofTheStormBonusWin') . '');
                            }
                            else
                            {
                                $winString .= ('&ms=' . $slotSettings->GetGameData('EyeofTheStormSym') . '&fsmul=' . $slotSettings->GetGameData('EyeofTheStormBonusMpl') . '&fsmax=' . $slotSettings->GetGameData('EyeofTheStormFreeGames') . '&fswin=' . $slotSettings->GetGameData('EyeofTheStormTotalWin') . '&fs=' . $slotSettings->GetGameData('EyeofTheStormCurrentFreeGame') . '&fsres=' . $slotSettings->GetGameData('EyeofTheStormBonusWin'));
                            }
                        ...
                        else
                        {
                            ...
                            $slotSettings->SetGameData('EyeofTheStormTotalWin', $totalWin);
                            $slotSettings->SetGameData('EyeofTheStormBonusWin', $totalWin);
                        }
                        ...
                        $response = 'tw=' . $slotSettings->GetGameData('EyeofTheStormBonusWin') . '&bstate=' . $slotSettings->GetGameData('EyeofTheStormBonusState') . '&balance=' . $Balance . '&index=' . $postData['index'] . '&balance_cash=' . $Balance . '&balance_bonus=0.00' . $winString . '&na=' . $spinType . '&stime=' . floor(microtime(true) * 1000) . '&sa=' . $reels['reel1'][3] . ',' . $reels['reel2'][3] . ',' . $reels['reel3'][3] . ',' . $reels['reel4'][3] . ',' . $reels['reel5'][3] . '&sb=' . $reels['reel1'][-1] . ',' . $reels['reel2'][-1] . ',' . $reels['reel3'][-1] . ',' . $reels['reel4'][-1] . ',' . $reels['reel5'][-1] . '&sh=3&c=0.01&sver=5&n_reel_set=0&counter=' . ((int)$postData['counter'] + 1) . '&l=10&s=' . $s . '&w=' . $totalWinRaw . '';
                        if( $slotSettings->GetGameData('EyeofTheStormFreeGames') <= $slotSettings->GetGameData('EyeofTheStormCurrentFreeGame') && $slotSettings->GetGameData('EyeofTheStormFreeGames') > 0 ) 
                        {
                            $slotSettings->SetGameData('EyeofTheStormTotalWin', 0);
                            $slotSettings->SetGameData('EyeofTheStormBonusWin', 0);
                        }
                        $response_log = '{"responseEvent":"spin","responseType":"' . $postData['slotEvent'] . '","serverResponse":{"Sym":' . $slotSettings->GetGameData('EyeofTheStormSym') . ',"BonusMpl":' . $slotSettings->GetGameData('EyeofTheStormBonusMpl') . ',"lines":' . $postData['slotLines'] . ',"bet":' . $postData['slotBet'] . ',"totalFreeGames":' . $slotSettings->GetGameData('EyeofTheStormFreeGames') . ',"currentFreeGames":' . $slotSettings->GetGameData('EyeofTheStormCurrentFreeGame') . ',"Balance":' . $Balance . ',"afterBalance":' . $slotSettings->GetBalance() . ',"totalWin":' . $totalWin . ',"bonusWin":' . $slotSettings->GetGameData('EyeofTheStormBonusWin') . ',"winLines":[],"Jackpots":' . $jsJack . ',"reelsSymbols":' . $jsSpin . '}}';
                        ...
                        if( $scattersCount >= 3 ) 
                        {
                            ...
                            {
                                $slotSettings->SetGameData('EyeofTheStormFreeBalance', $Balance);
                                $slotSettings->SetGameData('EyeofTheStormTotalWin', 0);
                                $slotSettings->SetGameData('EyeofTheStormBonusState', 0);
                                $slotSettings->SetGameData('EyeofTheStormBonusWin', $totalWin);
                            }
