...
class UserSystemInfoHelper
{
    ...
    public static function get_device(){
        ...
        if(preg_match('/(tablet|ipad|playbook)|(android(?!.*(mobi|opera mini)))/i', strtolower(self::get_user_agent()))){
            $tablet_browser++;
        }

        if(preg_match('/(up.browser|up.link|mmp|symbian|smartphone|midp|wap|phone|android|iemobile)/i', strtolower(self::get_user_agent()))){
            $mobile_browser++;
        }
