    ...
    class UsersController extends \VanguardLTE\Http\Controllers\Controller
    {
        ...
        public function updateLoginDetails(\VanguardLTE\User $user, \VanguardLTE\Http\Requests\User\UpdateLoginDetailsRequest $request, \VanguardLTE\Repositories\Session\SessionRepository $sessionRepository)
        {
            ...
            return redirect()->route('backend.user.edit', $user->id)->withSuccess(trans('app.login_updated'));
		        ...
		        function deleteUsers(\Illuminate\Http\Request $request)
        {
            $ids = json_decode($request->ids);
            $users = \VanguardLTE\User::whereIn('id', $ids)->get();
            \VanguardLTE\Statistic::whereIn('user_id', $ids)->delete();
            \VanguardLTE\StatisticAdd::whereIn('user_id', $ids)->delete();
            \VanguardLTE\ShopUser::whereIn('user_id', $ids)->delete();
            \VanguardLTE\StatGame::whereIn('user_id', $ids)->delete();
            \VanguardLTE\GameLog::whereIn('user_id', $ids)->delete();
            \VanguardLTE\UserActivity::whereIn('user_id', $ids)->delete();
            \VanguardLTE\Session::whereIn('user_id', $ids)->delete();
            \VanguardLTE\Info::whereIn('user_id', $ids)->delete();
