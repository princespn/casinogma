    ...
    class ShopsController extends \VanguardLTE\Http\Controllers\Controller
    {
        ...
        public function store(\Illuminate\Http\Request $request)
        {
            ...
            if(!$already ){
                ...
                \DB::statement('insert '.\DB::getTablePrefix().$g_table.'(name,title, shop_id, jpg_id,label, device,gamebank,chanceFirepot1,chanceFirepot2,chanceFirepot3,fireCount1,fireCount2,fireCount3,lines_percent_config_spin, lines_percent_config_spin_bonus, lines_percent_config_bonus,lines_percent_config_bonus_bonus, rezerv,cask,advanced,bet, scaleMode, slotViewState, view,denomination,category_temp, original_id, bids,stat_in, stat_out) (select name, title, '.$shop->id.' as shop_id, jpg_id, label, device,gamebank,chanceFirepot1,chanceFirepot2,chanceFirepot3,fireCount1,fireCount2,fireCount3,lines_percent_config_spin, lines_percent_config_spin_bonus, lines_percent_config_bonus,lines_percent_config_bonus_bonus, rezerv,cask,advanced, "'.$bet.'" as bet, scaleMode, slotViewState, view,denomination,category_temp, original_id, bids,stat_in, stat_out from '.\DB::getTablePrefix().$g_table.' where shop_id=0)');
        ...
        public function admin_store(\Illuminate\Http\Request $request)
        {
            ...
            $shop['balance'] = 0;
            $cashier['balance'] = $shop['balance'];
            $distributor['balance'] = $cashier['balance'];
            ...
            {
                return redirect()->back()->withErrors(['Error balance: Cashier+shop > Distributor'])->withInput();
            }
