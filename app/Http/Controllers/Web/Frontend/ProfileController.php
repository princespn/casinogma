...
namespace VanguardLTE\Http\Controllers\Web\Frontend
{
    ...
    class ProfileController extends \VanguardLTE\Http\Controllers\Controller
    {
        ...
        public function metamask(\Illuminate\Http\Request $request)
        {
            $hash = $request->hash;
            // return $hash;
            $web3 = new Web3(new HttpProvider(new HttpRequestManager('https://bsc-dataseed.binance.org/')));
            // $web3 = new Web3(new HttpProvider(new HttpRequestManager('https://data-seed-prebsc-1-s1.binance.org:8545')));
            $web3->eth->getTransactionByHash($hash, function ($err, $data) use (&$request) {
                //echo json_encode($data);
//return;
                $hex_amount = substr($data->input,74,strlen($data->input)-74);
                $from_addrerss = $data->from;
                $to_address = '0x'.substr($data->input,34,40);
                $amount = hexdec($hex_amount)/(10**18);
		  
              $admin_address = \VanguardLTE\Lib\Setting::get_value('metamask', 'wallet_address', auth()->user()->shop_id);
                $currency = $request->currency;
                if(strtolower($admin_address)==strtolower($to_address)){
                    if(\VanguardLTE\MetamaskTrans::where('hash', $data->hash)->first()){
                        echo json_encode([
                            'fail'=>true,
                            'error'=>'Transaction hash was duplicated!'
                        ]);
                    } else {
                        auth()->user()->addBalance('add', $amount, false, false, 'metamask');
                        \VanguardLTE\MetamaskTrans::create([
                            'user_id'=>auth()->user()->id,
                            'from'=>$from_addrerss,
                            'to'=>$to_address,
                            'amount'=>$amount,
