...
var GLOBALOBJ = {
    ...
    methods: {
            ...
            if (Object.keys(GLOBALOBJ.data.providerListFull).indexOf(providerId) === -1) {
                ...
                if (GLOBALOBJ.data.gameListToShow.length === 0) {
                    ...
                    for (var i = 0; i < GLOBALOBJ.data.gameListToShow.length; i++) {
                        if (i == 0 || i % 14 == 0) {
                            mainContentID++;
                            $(GLOBALOBJ.uiElements.mainContent).append("<div id='mainContentDIV" + mainContentID + "' class='row'></div>");
                        }
                        $('#mainContentDIV' + mainContentID).append(GLOBALOBJ.methods.createGameElement(GLOBALOBJ.data.gameListToShow[i]));
                        // $(GLOBALOBJ.uiElements.mainContent).append(GLOBALOBJ.methods.createGameElement(GLOBALOBJ.data.gameListToShow[i]));
                    }
                    $("#mainContent").owlCarousel({
                        items: 1,
                        nav: true,
                        smartSpeed: 1000,
                        dots: false
        ...
        createGameElement: function (gameData) {
            ...
            return $("<div/>", {
                // class: "col-12 m-auto game-container p-0 slot-game-item",
                class: "m-auto p-0 slot-game-item",
